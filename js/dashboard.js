...
// v6.6 — sesuai permintaan user: hanya untuk dilihat
// ============================================================
function bukaSuratSP(idx) {
  const sp = (window._spSayaCache || [])[idx];
  if (!sp) return;
  const user = typeof getSession === 'function' ? getSession() : {};
  const namaInst = (typeof getInstansi === 'function') ? (getInstansi().nama_instansi || 'Instansi') : 'Instansi';
  ...
  const tglTerbit    = sp.tanggal_terbit ? formatTanggal(sp.tanggal_terbit) : tglBerlaku;
  const noSurat      = sp.no_surat || sp.id_sp || '-';
  const dikeluarkan  = sp.dikeluarkan_oleh_nama || sp.dikeluarkan_oleh || 'HRD';
    ...
    <div style="background:#fff;border-radius:16px;width:100%;max-width:600px;
      ...
      <div style="padding:24px;font-family:'Times New Roman',serif;color:#1E293B;line-height:1.7">
        ...
        <div style="text-align:center;border-bottom:2px solid #1E293B;padding-bottom:12px;margin-bottom:18px">
          <div style="font-size:18px;font-weight:800;text-transform:uppercase;letter-spacing:.5px">
            ${namaInst}
          </div>
        </div>
        ...
        <div style="font-size:13px;margin-bottom:12px">
          <div>Kepada Yth.</div>
          <div style="font-weight:700">Sdr/i. ${user.nama_lengkap || '-'}</div>
          <div>${user.jabatan || '-'} - ${user.departemen || '-'}</div>
          <div>Di Tempat</div>
        </div>
        ...
        <div style="font-size:13px;margin-bottom:14px;text-align:justify">
          Sehubungan dengan evaluasi kehadiran dan kedisiplinan kerja di lingkungan
          ${namaInst}, bersama ini kami sampaikan bahwa Saudara/i:
        </div>
        ...
        <div style="background:#EFF6FF;border:1px solid #2D6CDF;border-radius:6px;
          ...
          <table style="width:100%;font-size:13px">
            <tr><td style="width:140px;font-weight:700;padding:3px 0">Nama Lengkap</td>
                <td style="padding:3px 0">: ${user.nama_lengkap || '-'}</td></tr>
            <tr><td style="font-weight:700;padding:3px 0">NIK</td>
                <td style="padding:3px 0">: ${user.nik || '-'}</td></tr>
            <tr><td style="font-weight:700;padding:3px 0">Jabatan</td>
                <td style="padding:3px 0">: ${user.jabatan || '-'}</td></tr>
            <tr><td style="font-weight:700;padding:3px 0">Departemen</td>
                <td style="padding:3px 0">: ${user.departemen || '-'}</td></tr>
          </table>
        </div>
        ...
        <div style="margin-top:24px;display:flex;justify-content:flex-end">
          <div style="text-align:center;font-size:13px">
            <div>${namaInst}, ${tglTerbit}</div>
            <div style="margin:50px 0 4px;font-weight:700;text-decoration:underline">${dikeluarkan}</div>
            <div style="font-size:12px">HRD / Pimpinan</div>
          </div>
        </div>
