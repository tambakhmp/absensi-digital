...
async function loadDashboardAdminV3() {
  try {
    ...
    if (s.chart_6_bulan) try { renderChart6Bulan(s.chart_6_bulan); } catch(e){}
    if (s.ultah_hari_ini?.length > 0) _renderUltah(s.ultah_hari_ini);
...
async function tampilExportAbsensi() {
  ...
  showModal('📊 Export Rekap Excel',
    `<div class="form-group" style="margin-top:8px"><label class="form-label">Bulan</label>
    ...
    <div class="form-group"><label class="form-label">Tahun</label>
      <input type="number" class="form-control" id="exp-thn" value="${now.getFullYear()}"></div>`,
    async()=>exportRekapExcel(document.getElementById('exp-bln')?.value,document.getElementById('exp-thn')?.value),
    '📊 Download Excel');
}
...
function tampilFormTambahSP(){
  showModal('⚠️ Tambah SP Manual',
    `<div class="form-group" style="margin-top:8px"><label class="form-label">ID Karyawan *</label>
    <input type="text" class="form-control" id="sp-idk" placeholder="KRY001"></div>
    <div class="form-group"><label class="form-label">Jenis SP *</label>
    <select class="form-control" id="sp-jns">
      <option value="SP1">SP1 (1 bulan)</option><option value="SP2">SP2 (2 bulan)</option><option value="SP3">SP3 (3 bulan)</option>
    </select></div>
    <div class="form-group"><label class="form-label">Alasan *</label>
    <textarea class="form-control" id="sp-alasan" rows="3" placeholder="Jelaskan alasan..."></textarea></div>
    <div class="form-group"><label class="form-label">Total Hari Alfa</label>
    <input type="number" class="form-control" id="sp-alfa" value="3" min="0"></div>`,
    async()=>{
      try{const r=await callAPI('tambahSP',{
        id_karyawan:document.getElementById('sp-idk')?.value?.trim(),
...
async function loadJadwalAdmin(){
  ...
  try{
    const mul=fromInputDate(document.getElementById('jdw-view-mul')?.value||new Date().toISOString().split('T')[0]);
    const sel=fromInputDate(document.getElementById('jdw-view-sel')?.value||new Date().toISOString().split('T')[0]);
...
function _siUrl(key,label,map){
  const val=map[key]||'';
  const normUrl=val?_normDrive(val):'';
