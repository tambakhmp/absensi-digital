...
async function renderRankingAdminFull(container) {
  ...
  container.innerHTML=`
    <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;margin-bottom:16px">
      ...
      <div style="display:flex;gap:8px">
        <select class="form-control" id="rank-bln" style="width:140px" onchange="loadRankingAdmin()">
          ${Array.from({length:12},(_,i)=>`<option value="${i+1}" ${i===now.getMonth()?'selected':''}>${bulanNama(i+1)}</option>`).join('')}
        </select>
        <input type="number" class="form-control" id="rank-thn" value="${now.getFullYear()}" style="width:85px">
        <button class="btn btn--primary" style="font-size:13px" onclick="hitungRankingAdmin()">⚡ Hitung</button>
      </div>
    </div>
  ...
  try {
    ...
    function rCard(r) {
      ...
      return `<div style="display:flex;align-items:center;gap:12px;padding:10px 0;
          ...
          <div style="flex:1;min-width:0">
            ...
            ${r.predikat?`<span class="rank-badge rank-badge--${BMAP[r.predikat]||''}"
              style="margin-top:3px;display:inline-flex">${r.predikat}</span>`:''}
          </div>
          <div style="text-align:right;flex-shrink:0">
            <div style="font-size:13px;font-weight:700;color:var(--color-primer)">#${r.peringkat}</div>
            <div style="font-size:11px;color:#1A9E74">✅ ${r.total_hadir||0} hr</div>
            <div style="font-size:11px;color:#E53E3E">❌ ${r.total_alfa||0} alfa</div>
          </div>
        </div>`;
    }
    ...
    el.innerHTML=`
      ...
      ${tengah.length?`<div class="card"><p style="font-size:12px;font-weight:700;color:#64748B;
        ...
        ${tengah.map(r=>`<div style="display:flex;align-items:center;gap:10px;padding:7px 0;
          border-bottom:1px solid #F1F5F9">
          <span style="font-weight:700;color:#64748B;width:28px;text-align:right">#${r.peringkat}</span>
          <div style="flex:1;font-size:13px;font-weight:600">${r.nama_karyawan}</div>
          <div style="font-size:12px;color:#64748B">${r.skor_kehadiran||0} poin</div>
          <div style="font-size:12px;color:#1A9E74">✅${r.total_hadir||0}</div>
          <div style="font-size:12px;color:#E53E3E">❌${r.total_alfa||0}</div>
        </div>`).join('')}</div>`:''}
...
async function hitungRankingAdmin(){
  showModal('⚡ Hitung Ranking?','Ranking akan dihitung ulang berdasarkan data absensi.',
    async()=>{
      showToast('Menghitung...','info',5000);
      try{const r=await callAPI('hitungRanking',{});showToast(r.message,'success',5000);loadRankingAdmin();}
      catch(e){showToast(e.message,'error');}
    },'⚡ Hitung');
}
  ...
  el.innerHTML=`<div style="overflow-x:auto"><table class="simple-table" style="min-width:400px;font-size:12px">
    <thead><tr><th>Karyawan</th>
      ${tanggals.slice(0,14).map(t=>{
        const p=t.split('/'); const d=new Date(parseInt(p[2]),parseInt(p[1])-1,parseInt(p[0]));
    ...
    ${(peserta||[]).map(k=>`<tr>
      ...
      ${tanggals.slice(0,14).map(t=>{
        ...
        const kode=e?.kode||'-'; const c=KC[kode]||'#94A3B8';
        return `<td style="text-align:center;padding:4px">
          <span style="background:${c}22;color:${c};border:1px solid ${c}44;
            padding:2px 6px;border-radius:6px;font-weight:700;font-size:11px">${kode}</span></td>`;
      }).join('')}
    </tr>`).join('')}
    </tbody></table></div>
...
${map['ucapan_ulang_tahun_template']||''}</textarea>
        ...
        <div class="form-group">
          <label class="form-label">📧 Email Notifikasi (v6.5)</label>
          <select class="form-control" id="set-aktif_email_notif">
            <option value="true" ${String(map['aktif_email_notif']).toLowerCase()==='true'?'selected':''}>
              ✅ Aktif — Kirim email notif (pengajuan approve/reject, reminder absen keluar)</option>
            <option value="false" ${String(map['aktif_email_notif']).toLowerCase()!=='true'?'selected':''}>
              ❌ Nonaktif — Tidak ada email notif</option>
          </select>
          <p class="form-hint">Pastikan karyawan punya email valid di profilnya. Quota: 100 email/hari (akun gratis).</p>
        </div>
      </div>
      ...
      <div class="card">
        <h3 style="font-size:14px;font-weight:700;color:#64748B;text-transform:uppercase;
          letter-spacing:.6px;margin-bottom:14px">⏰ Jam Kerja & Aturan Absensi</h3>
        <div style="background:#FFF8DC;border-radius:8px;padding:12px;font-size:12px;
          color:#B7791F;margin-bottom:14px;line-height:1.7">
          📌 <strong>Cara kerja:</strong><br>
          • Karyawan <strong>shift security</strong> → pakai jam shift masing-masing (Pagi/Sore/Malam)<br>
          • Karyawan <strong>non-shift</strong> → pakai jam masuk/keluar default di bawah<br>
          • <strong>Dinas luar</strong> → aturan sama, zona waktu otomatis dari GPS (WIB/WITA/WIT)<br>
          • <strong>Admin</strong> bisa input absensi manual untuk karyawan yang lewat batas
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
          <div class="form-group">
            <label class="form-label">Jam Masuk Default</label>
            <input type="time" class="form-control" id="set-jam_masuk_default"
              value="${map['jam_masuk_default']||'08:00'}">
            <p class="form-hint">Untuk karyawan non-shift</p>
          </div>
          <div class="form-group">
            <label class="form-label">Jam Keluar Default</label>
            <input type="time" class="form-control" id="set-jam_keluar_default"
              value="${map['jam_keluar_default']||'16:00'}">
            <p class="form-hint">Untuk karyawan non-shift</p>
          </div>
        </div>
        <div style="background:#F1F5F9;border-radius:8px;padding:12px;margin:12px 0">
          <div style="font-size:12px;font-weight:700;color:#475569;margin-bottom:10px;
            text-transform:uppercase;letter-spacing:.5px">Aturan Batas Waktu Absen</div>
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px">
            <div class="form-group" style="margin-bottom:0">
              <label class="form-label" style="font-size:12px">Toleransi Terlambat (menit)</label>
              <input type="number" class="form-control" id="set-toleransi_terlambat_menit"
                value="${map['toleransi_terlambat_menit']||map['toleransi_terlambat_default']||15}" min="0" max="60">
              <p class="form-hint" style="font-size:11px">Di atas ini = terlambat</p>
            </div>
            <div class="form-group" style="margin-bottom:0">
              <label class="form-label" style="font-size:12px">Batas Absen Masuk (menit)</label>
              <input type="number" class="form-control" id="set-batas_absen_masuk_menit"
                value="${map['batas_absen_masuk_menit']||60}" min="15" max="240">
              <p class="form-hint" style="font-size:11px">Di atas ini = ditolak</p>
            </div>
            <div class="form-group" style="margin-bottom:0">
              <label class="form-label" style="font-size:12px">Aturan Absen Keluar</label>
              <div class="form-control" style="background:#F8FAFC;cursor:not-allowed;color:#64748B;font-size:12px;line-height:1.4;padding:8px 10px">
                Tidak boleh absen keluar sebelum jam keluar yang ditentukan
              </div>
              <p class="form-hint" style="font-size:11px">Otomatis (non-editable)</p>
            </div>
          </div>
        </div>
        <div style="background:#EFF6FF;border-radius:8px;padding:10px;font-size:11px;
          color:#1E40AF;line-height:1.6">
          <strong>💡 Contoh:</strong> Jam masuk 08:00, toleransi 15mnt, batas 60mnt →
          Absen 07:00–08:15 = hadir, 08:16–09:00 = terlambat, setelah 09:00 = ditolak.
          Jam keluar 16:00 → absen keluar tidak bisa sebelum 16:00.
        </div>
      </div>
      ...
      <!-- ⚠️ ZONA BAHAYA (hanya superadmin) -->
      <div class="card" style="margin-top:24px;border:2px solid #FCA5A5;background:#FEF2F2">
        <h3 style="font-size:14px;font-weight:700;color:#991B1B;text-transform:uppercase;
          letter-spacing:.6px;margin-bottom:14px">⚠️ Zona Bahaya</h3>
        <div style="background:#FEF3C7;border:1px solid #F59E0B;border-radius:8px;padding:12px;
          font-size:12px;color:#92400E;margin-bottom:16px;line-height:1.7">
          🛡️ <strong>Area ini hanya untuk Super Admin.</strong> Aksi di sini
          tidak dapat dibatalkan. Selalu <strong>backup data</strong> sebelum reset.
        </div>
          ...
          <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap">
            <div style="flex:1;min-width:200px">
              <div style="font-weight:700;font-size:14px;color:#1E293B;margin-bottom:4px">
                💾 Backup Semua Data</div>
              <div style="font-size:12px;color:#64748B;line-height:1.6">
                Export semua sheet ke file JSON di Google Drive
                (folder <code>07 - Backup Data</code>). Aman untuk dijalankan kapan saja.
              </div>
            </div>
            <button class="btn btn--secondary" onclick="backupSemuaDataAdmin()"
              id="btn-backup-data" style="white-space:nowrap">
              <div class="spinner-btn"></div><span class="btn-text">💾 Backup Sekarang</span>
            </button>
          </div>
          <div id="backup-result" style="margin-top:10px;display:none"></div>
        </div>

        <!-- Factory Reset -->
        <div style="background:#fff;border-radius:8px;padding:16px;border:2px solid #FCA5A5">
          <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap">
            <div style="flex:1;min-width:200px">
              <div style="font-weight:700;font-size:14px;color:#991B1B;margin-bottom:4px">
                🗑️ Factory Reset (HAPUS SEMUA DATA)</div>
              <div style="font-size:12px;color:#7F1D1D;line-height:1.6">
                Menghapus <strong>semua karyawan</strong> (kecuali akun Anda),
                <strong>semua absensi, pengajuan, lembur, SP, pengumuman, jadwal,
                riwayat login, ranking, device, cuti khusus</strong>, dan
                <strong>semua foto/file di Drive</strong>. Data master (shift,
                lokasi kantor, harga lembur, pengaturan) tetap.
                <br><strong style="color:#B91C1C">⚠️ Aksi ini TIDAK BISA DIBATALKAN.</strong>
              </div>
            </div>
            <button class="btn" onclick="bukaDialogReset()"
              style="background:#DC2626;color:#fff;white-space:nowrap;border:none">
              🗑️ Factory Reset
            </button>
          </div>
        </div>
...
async function backupSemuaDataAdmin() {
  if (!confirm('Backup semua data sekarang?\n\nProses ini bisa memakan 10-30 detik tergantung jumlah data.')) return;
  const btn = document.getElementById('btn-backup-data');
  const resEl = document.getElementById('backup-result');
  if (btn) { btn.disabled = true; btn.classList.add('loading'); }
  try {
    const r = await callAPI('backupSemuaData', {});
    if (resEl) {
      resEl.style.display = 'block';
      resEl.innerHTML = `<div style="background:#ECFDF5;border:1px solid #10B981;border-radius:8px;
        padding:10px 14px;font-size:12px;color:#065F46;line-height:1.6">
        ✅ <strong>Backup berhasil!</strong><br>
        File: <code>${r.nama_file}</code> (${r.ukuran_kb} KB, ${r.total_rows} rows, ${r.total_sheet} sheet)<br>
        <a href="${r.url}" target="_blank" style="color:#059669;font-weight:600">📂 Buka di Drive</a>
        &nbsp;·&nbsp;
        <a href="${r.download_url}" target="_blank" style="color:#059669;font-weight:600">⬇️ Download JSON</a>
      </div>`;
    }
    showToast('✅ Backup berhasil!', 'success', 5000);
  } catch(e) {
    showToast('❌ Backup gagal: ' + e.message, 'error', 8000);
    if (resEl) {
      resEl.style.display = 'block';
      resEl.innerHTML = `<div style="background:#FEF2F2;border:1px solid #FCA5A5;border-radius:8px;
        padding:10px 14px;font-size:12px;color:#991B1B">❌ ${e.message}</div>`;
    }
  } finally {
    if (btn) { btn.disabled = false; btn.classList.remove('loading'); }
  }
...
// ─── FACTORY RESET DIALOG (Super Admin) ────────────────────
function bukaDialogReset() {
  const modal = document.createElement('div');
  modal.id = 'modal-factory-reset';
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:16px;padding:24px;width:100%;max-width:480px;
      max-height:92vh;overflow-y:auto;animation:fadeInScale .2s ease;border:3px solid #DC2626">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
        <h3 style="font-size:17px;font-weight:700;color:#991B1B;margin:0">⚠️ FACTORY RESET</h3>
        <button onclick="document.getElementById('modal-factory-reset').remove()"
          style="background:#F1F5F9;border:none;border-radius:50%;width:30px;height:30px;cursor:pointer">✕</button>
      </div>
      <div style="background:#FEF2F2;border:1px solid #FCA5A5;border-radius:10px;padding:14px;
        font-size:13px;color:#991B1B;margin-bottom:16px;line-height:1.7">
        <strong>⚠️ PERINGATAN KERAS</strong><br>
        Tindakan ini akan <strong>menghapus permanen</strong>:
        <ul style="margin:6px 0 0 18px;padding:0">
          <li>Semua data karyawan (kecuali akun Anda)</li>
          <li>Semua absensi, pengajuan, lembur, SP, pengumuman</li>
          <li>Semua jadwal, riwayat login, ranking, device</li>
          <li>Semua foto di Drive (absen, profil, TTD, pendukung)</li>
        </ul>
        <strong style="display:block;margin-top:8px">Tidak bisa dibatalkan!</strong>
      </div>

      <div class="form-group">
        <label class="form-label" style="font-size:13px">
          1. Ketik persis: <code style="background:#FEF2F2;color:#991B1B;padding:2px 6px;border-radius:4px">HAPUS SEMUA DATA</code>
        </label>
        <input type="text" class="form-control" id="reset-konfirmasi"
          placeholder="Ketik: HAPUS SEMUA DATA" autocomplete="off"
          style="font-family:monospace;letter-spacing:.5px">
      </div>

      <div class="form-group">
        <label class="form-label" style="font-size:13px">2. Masukkan password Super Admin Anda:</label>
        <input type="password" class="form-control" id="reset-password"
          placeholder="Password" autocomplete="current-password">
      </div>

      <div id="reset-err" style="display:none;background:#FEF2F2;border:1px solid #FCA5A5;
        border-radius:8px;padding:10px 12px;color:#991B1B;font-size:13px;margin-bottom:12px"></div>

      <div style="display:flex;gap:8px">
        <button class="btn btn--ghost" onclick="document.getElementById('modal-factory-reset').remove()"
          style="flex:1">Batal</button>
        <button class="btn" id="btn-confirm-reset" onclick="eksekusiFactoryReset()"
          style="flex:1;background:#DC2626;color:#fff;border:none">
          <div class="spinner-btn"></div><span class="btn-text">🗑️ Hapus Semua Data</span>
        </button>
      </div>
    </div>`;
  document.body.appendChild(modal);
  setTimeout(() => document.getElementById('reset-konfirmasi')?.focus(), 100);
}

async function eksekusiFactoryReset() {
  const btn = document.getElementById('btn-confirm-reset');
  const err = document.getElementById('reset-err');
  const konfirmasi = document.getElementById('reset-konfirmasi')?.value?.trim() || '';
  const password = document.getElementById('reset-password')?.value || '';
  if (err) err.style.display = 'none';

  // Validasi client-side dulu (biar user tidak tunggu lama)
  if (konfirmasi !== 'HAPUS SEMUA DATA') {
    if (err) { err.style.display = 'block'; err.textContent = '⚠️ Teks konfirmasi harus persis: HAPUS SEMUA DATA'; }
    return;
  }
  if (!password) {
    if (err) { err.style.display = 'block'; err.textContent = '⚠️ Password wajib diisi'; }
    return;
  }

  // Konfirmasi final
  if (!confirm('BENAR-BENAR mau HAPUS SEMUA DATA?\n\nKlik OK untuk lanjut, atau Cancel untuk batal.')) return;

  if (btn) { btn.disabled = true; btn.classList.add('loading'); }
  try {
    const pwHash = await sha256(password);
    const r = await callAPI('factoryReset', {
      konfirmasi: konfirmasi,
      password_hash: pwHash
    });
    document.getElementById('modal-factory-reset')?.remove();
    // Tampilkan hasil dulu, baru logout setelah 3 detik
    showToast('✅ Sistem berhasil di-reset. Anda akan di-logout dalam 3 detik...', 'success', 3500);
    setTimeout(() => {
      localStorage.removeItem('absensi_token');
      localStorage.removeItem('absensi_user');
      localStorage.removeItem('_brand');
...
async function simpanPengaturanAdmin(){
  ...
  try{
    const keys=['nama_instansi','singkatan_instansi','alamat_instansi','telepon_instansi',
      ...
      // Jam Kerja & Aturan Absensi v6
      'jam_masuk_default','jam_keluar_default','toleransi_terlambat_menit',
      'batas_absen_masuk_menit'];
    const settings={};
  ...
  container.innerHTML=`
    ...
    <div class="card">
      <h3 style="font-size:15px;font-weight:700;margin-bottom:14px">
        📈 Rekap Tahunan & Grafik Trend
      </h3>
      <p style="font-size:12px;color:#64748B;margin-bottom:12px">
        Lihat ringkasan performa kehadiran setahun penuh dengan grafik bulanan,
        top 5 dan bottom 5 karyawan.
      </p>
      <div style="display:flex;gap:10px;align-items:end;flex-wrap:wrap">
        <div class="form-group" style="margin-bottom:0;min-width:120px">
          <label class="form-label">Tahun</label>
          <input type="number" class="form-control" id="rt-tahun"
            value="${now.getFullYear()}" min="2020" max="${now.getFullYear()+1}">
        </div>
        <button class="btn btn--primary" style="flex:1;min-width:140px"
          onclick="bukaRekapTahunan()">📈 Lihat Rekap</button>
      </div>
    </div>
    ...
    <div class="card">
      <h3 style="font-size:15px;font-weight:700;margin-bottom:14px">
        🗄️ Arsip Data Absensi
      </h3>
      <p style="font-size:12px;color:#64748B;margin-bottom:12px">
        Data absensi lebih dari 12 bulan otomatis dipindah ke arsip per tahun
        untuk menjaga performa sistem.
      </p>
      <button class="btn btn--ghost btn--full" onclick="bukaArsipData()">
        🗄️ Buka Arsip Data
      </button>
    </div>

    <!-- PETA LOKASI ABSEN (v6.5) -->
    <div class="card">
      <h3 style="font-size:15px;font-weight:700;margin-bottom:14px">
        🗺️ Peta Lokasi Absen
      </h3>
      <p style="font-size:12px;color:#64748B;margin-bottom:12px">
        Lihat semua titik lokasi absen karyawan di peta interaktif. Klik marker
        untuk lihat detail (nama, foto selfie, jam absen).
      </p>
      <div style="display:flex;gap:8px;align-items:end;flex-wrap:wrap">
        <div class="form-group" style="margin-bottom:0;min-width:140px">
          <label class="form-label">Tanggal</label>
          <input type="date" class="form-control" id="peta-tgl"
            value="${new Date().toISOString().split('T')[0]}">
        </div>
        <button class="btn btn--primary" style="flex:1;min-width:140px"
          onclick="bukaPetaLokasi()">🗺️ Lihat Peta</button>
      </div>
    </div>`;
...
// REKAP TAHUNAN (v6.4)
// ============================================================
async function bukaRekapTahunan() {
  const tahun = parseInt(document.getElementById('rt-tahun')?.value) || new Date().getFullYear();
  const modal = document.createElement('div');
  modal.id = 'modal-rekap-tahunan';
  modal.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:9000;' +
    'display:flex;align-items:center;justify-content:center;padding:16px;overflow-y:auto';
  modal.innerHTML = `
    <div style="background:#fff;border-radius:16px;padding:0;width:100%;max-width:860px;
      max-height:92vh;overflow-y:auto;animation:fadeInScale .2s ease;display:flex;flex-direction:column">
      <div style="padding:18px 24px;border-bottom:1px solid #E2E8F0;display:flex;
        justify-content:space-between;align-items:center;position:sticky;top:0;background:#fff;z-index:2">
        <h3 style="font-size:17px;font-weight:700;margin:0">📈 Rekap Tahunan ${tahun}</h3>
        <button onclick="document.getElementById('modal-rekap-tahunan').remove()"
          style="background:#F1F5F9;border:none;border-radius:50%;width:32px;height:32px;cursor:pointer">✕</button>
      </div>
      <div id="rt-content" style="padding:20px 24px">
        <div style="text-align:center;padding:40px;color:#94A3B8">
          <div class="spinner-btn" style="display:inline-block"></div>
          Memuat data tahun ${tahun}...
        </div>
      </div>
    </div>`;
  document.body.appendChild(modal);
  ...
  try {
    const data = await callAPI('getRekapTahunan', { tahun: tahun });
    _renderRekapTahunan(data);
  } catch(e) {
    const el = document.getElementById('rt-content');
    if (el) el.innerHTML = `<div style="background:#FEF2F2;border:1px solid #FCA5A5;border-radius:8px;
      padding:14px;color:#991B1B">❌ ${e.message}</div>`;
  }
}

function _renderRekapTahunan(data) {
  const el = document.getElementById('rt-content');
  if (!el) return;

  const t = data.total || {};
  const totalAll = (t.hadir||0)+(t.terlambat||0)+(t.alfa||0)+(t.izin||0)+(t.sakit||0)+(t.cuti||0)+(t.dinas_luar||0);
  const hadirEfektif = (t.hadir||0)+(t.terlambat||0)+(t.dinas_luar||0);
  const persenHadirThn = totalAll > 0 ? Math.round((hadirEfektif/totalAll)*100) : 0;

  el.innerHTML = `
    <!-- Ringkasan -->
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:10px;margin-bottom:20px">
      ${[
        ['👥','Karyawan Aktif',data.total_karyawan,'#2D6CDF'],
        ['📅','Rata² Hari Kerja',data.rata_rata_hari_kerja,'#0891B2'],
        ['✅','Total Hadir',hadirEfektif,'#1A9E74'],
        ['📊','Tingkat Kehadiran',persenHadirThn+'%', persenHadirThn>=85?'#1A9E74':persenHadirThn>=70?'#D97706':'#E53E3E']
      ].map(([ic,lb,v,clr])=>`<div style="background:${clr}11;border-left:3px solid ${clr};
        border-radius:8px;padding:10px 12px">
        <div style="font-size:11px;color:#64748B;margin-bottom:2px">${ic} ${lb}</div>
        <div style="font-size:18px;font-weight:700;color:${clr}">${v}</div>
      </div>`).join('')}
    </div>

    <!-- Grafik 12 bulan -->
    <div class="card" style="padding:14px;margin-bottom:20px">
      <h4 style="font-size:14px;font-weight:700;margin-bottom:10px">📊 Trend per Bulan</h4>
      <canvas id="chart-tahunan" style="max-height:280px"></canvas>
    </div>

    <!-- Top 5 -->
    <div class="card" style="padding:14px;margin-bottom:14px">
      <h4 style="font-size:14px;font-weight:700;margin-bottom:10px;color:#065F46">
        🏆 Top 5 Karyawan Terbaik
      </h4>
      ${(data.top5 || []).map((k,i)=>`
        <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #F1F5F9">
          <div style="width:28px;height:28px;border-radius:50%;background:${i===0?'#FCD34D':i===1?'#E5E7EB':i===2?'#FED7AA':'#F1F5F9'};
            display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;color:#1E293B">
            ${i+1}</div>
          <div style="flex:1;min-width:0">
            <div style="font-weight:600;font-size:13px">${k.nama}</div>
            <div style="font-size:11px;color:#64748B">${k.jabatan} · ${k.departemen}</div>
          </div>
          <div style="text-align:right">
            <div style="font-size:12px;color:#065F46;font-weight:700">${k.hadir_total} hadir · ${k.alfa} alfa</div>
            <div style="font-size:11px;color:#64748B">${k.persen_hadir}% kehadiran</div>
          </div>
        </div>
      `).join('') || '<div style="text-align:center;color:#94A3B8;padding:20px">Belum ada data</div>'}
    </div>

    <!-- Bottom 5 -->
    <div class="card" style="padding:14px;margin-bottom:14px">
      <h4 style="font-size:14px;font-weight:700;margin-bottom:10px;color:#991B1B">
        ⚠️ 5 Karyawan Perlu Perhatian
      </h4>
      ${(data.bottom5 || []).map((k,i)=>`
        <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #F1F5F9">
          <div style="width:28px;height:28px;border-radius:50%;background:#FEE2E2;
            display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;color:#991B1B">
            ${i+1}</div>
          <div style="flex:1;min-width:0">
            <div style="font-weight:600;font-size:13px">${k.nama}</div>
            <div style="font-size:11px;color:#64748B">${k.jabatan} · ${k.departemen}</div>
          ...
          <div style="text-align:right">
            <div style="font-size:12px;color:#991B1B;font-weight:700">${k.alfa} alfa · ${k.terlambat} telat</div>
            <div style="font-size:11px;color:#64748B">${k.persen_hadir}% kehadiran</div>
          </div>
        </div>
      `).join('') || '<div style="text-align:center;color:#94A3B8;padding:20px">Belum ada data</div>'}
    ...
    <div style="display:flex;gap:8px;margin-bottom:8px">
      <button class="btn btn--ghost" style="flex:1" onclick="document.getElementById('modal-rekap-tahunan').remove()">Tutup</button>
      <button class="btn btn--primary" style="flex:1" onclick="exportRekapTahunanExcel(${data.tahun})">📊 Export Excel</button>
    </div>`;

  // Render grafik
  setTimeout(() => _renderChartTahunan(data.per_bulan), 100);
}

function _renderChartTahunan(perBulan) {
  if (typeof Chart === 'undefined' || !perBulan) return;
  const ctx = document.getElementById('chart-tahunan');
  if (!ctx) return;
  ...
  try {
    new Chart(ctx, {
      type: 'bar',
      data: {
        labels: perBulan.map(b => b.nama),
        datasets: [
          { label:'Hadir', data:perBulan.map(b=>b.hadir+b.terlambat+b.dinas_luar), backgroundColor:'#1A9E74' },
          { label:'Terlambat', data:perBulan.map(b=>b.terlambat), backgroundColor:'#D97706' },
          { label:'Alfa', data:perBulan.map(b=>b.alfa), backgroundColor:'#E53E3E' },
          { label:'Izin/Sakit/Cuti', data:perBulan.map(b=>b.izin+b.sakit+b.cuti), backgroundColor:'#6C63FF' }
        ]
      },
      options: {
        responsive: true,
...
async function exportRekapTahunanExcel(tahun) {
  showToast('Mempersiapkan Excel...', 'info', 2000);
  const ok = await _ensureXLSX();
  if (!ok) { showToast('Library Excel tidak tersedia', 'error'); return; }
  try {
    const data = await callAPI('getRekapTahunan', { tahun: tahun });
    const nama = data.instansi?.nama_instansi || 'Instansi';

    // Sheet 1: Ringkasan per Karyawan
    const aoa = [
      [nama],
      [`REKAP TAHUNAN KARYAWAN - TAHUN ${tahun}`],
      [],
      ['No','Nama','Jabatan','Departemen','Hadir','Terlambat','Alfa','Izin','Sakit','Cuti','Dinas Luar','Total Hadir','% Hadir']
    ];
    (data.per_karyawan || []).forEach((k, i) => {
      aoa.push([
        i+1, k.nama, k.jabatan, k.departemen,
        k.hadir, k.terlambat, k.alfa, k.izin, k.sakit, k.cuti, k.dinas_luar,
        k.hadir_total, k.persen_hadir
      ]);
    });

    const ws = XLSX.utils.aoa_to_sheet(aoa);
    ws['!cols'] = [{wch:4},{wch:24},{wch:18},{wch:16},{wch:7},{wch:10},{wch:6},{wch:6},{wch:6},{wch:6},{wch:10},{wch:10},{wch:8}];
    ws['!merges'] = [
      {s:{r:0,c:0},e:{r:0,c:12}},
      {s:{r:1,c:0},e:{r:1,c:12}}
    ];
    // Style header
    const sJudul = {font:{bold:true,sz:14,color:{rgb:'FFFFFF'}},fill:{patternType:'solid',fgColor:{rgb:'1E3A5F'}},alignment:{horizontal:'center'}};
    const sSub   = {font:{bold:true,sz:11,color:{rgb:'FFFFFF'}},fill:{patternType:'solid',fgColor:{rgb:'2D6CDF'}},alignment:{horizontal:'center'}};
    const sHead  = {font:{bold:true,sz:10,color:{rgb:'FFFFFF'}},fill:{patternType:'solid',fgColor:{rgb:'1E3A5F'}},alignment:{horizontal:'center'},border:{top:{style:'thin'},bottom:{style:'thin'},left:{style:'thin'},right:{style:'thin'}}};
    const enc = (r,c) => XLSX.utils.encode_cell({r,c});
    for (let c=0;c<13;c++) {
      if (ws[enc(0,c)]) ws[enc(0,c)].s = sJudul;
      if (ws[enc(1,c)]) ws[enc(1,c)].s = sSub;
      if (ws[enc(3,c)]) ws[enc(3,c)].s = sHead;
    }
    // Data rows
    for (let i=0;i<(data.per_karyawan||[]).length;i++) {
      const r = 4+i;
      for (let c=0;c<13;c++) {
        if (ws[enc(r,c)]) {
          ws[enc(r,c)].s = {
    ...
    // Sheet 2: Per Bulan
    const aoa2 = [
      ['Bulan','Hadir','Terlambat','Alfa','Izin','Sakit','Cuti','Dinas Luar']
    ];
    (data.per_bulan || []).forEach(b => {
      aoa2.push([b.nama, b.hadir, b.terlambat, b.alfa, b.izin, b.sakit, b.cuti, b.dinas_luar]);
    });
    const ws2 = XLSX.utils.aoa_to_sheet(aoa2);
    ws2['!cols'] = [{wch:10},{wch:8},{wch:10},{wch:8},{wch:8},{wch:8},{wch:8},{wch:10}];
    for (let c=0;c<8;c++) if (ws2[enc(0,c)]) ws2[enc(0,c)].s = sHead;

    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Per Karyawan');
    XLSX.utils.book_append_sheet(wb, ws2, 'Per Bulan');
    XLSX.writeFile(wb, `Rekap_Tahunan_${tahun}.xlsx`);
    showToast('✅ Rekap tahunan berhasil diunduh!', 'success');
  } catch(e) {
...
// ARSIP DATA (v6.4)
// ============================================================
async function bukaArsipData() {
  const modal = document.createElement('div');
  modal.id = 'modal-arsip';
  modal.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:9000;' +
    'display:flex;align-items:center;justify-content:center;padding:16px;overflow-y:auto';
  modal.innerHTML = `
    <div style="background:#fff;border-radius:16px;padding:24px;width:100%;max-width:640px;
      max-height:92vh;overflow-y:auto;animation:fadeInScale .2s ease">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
        <h3 style="font-size:17px;font-weight:700;margin:0">🗄️ Arsip Data Absensi</h3>
        <button onclick="document.getElementById('modal-arsip').remove()"
          style="background:#F1F5F9;border:none;border-radius:50%;width:32px;height:32px;cursor:pointer">✕</button>
      </div>
      <div id="arsip-content">
        <div style="text-align:center;padding:40px;color:#94A3B8">
          <div class="spinner-btn" style="display:inline-block"></div> Memuat...
        </div>
      </div>
    </div>`;
  document.body.appendChild(modal);
  ...
  try {
    const data = await callAPI('getArsipList', {});
    _renderArsipList(data.list || []);
  } catch(e) {
...
function _renderArsipList(list) {
  const el = document.getElementById('arsip-content');
  if (!el) return;

  if (!list.length) {
    el.innerHTML = `<div style="background:#F0F9FF;border:1px solid #38BDF8;border-radius:10px;
      padding:20px;text-align:center;color:#0369A1;font-size:13px">
      <div style="font-size:40px;margin-bottom:8px">📭</div>
      Belum ada data arsip.<br>
      <span style="font-size:12px;color:#64748B">Archive otomatis jalan tanggal 1 tiap bulan.</span>
    </div>`;
    return;
  }

  el.innerHTML = `<div style="font-size:13px;color:#64748B;margin-bottom:12px">
    Data absensi lebih dari 12 bulan dipindah ke arsip per tahun:
  </div>
  ${list.map(a => `
    <div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:8px;padding:12px 14px;
      margin-bottom:8px;display:flex;justify-content:space-between;align-items:center;gap:10px">
      <div>
        <div style="font-weight:700;font-size:15px;color:#1E293B">Tahun ${a.tahun}</div>
        <div style="font-size:12px;color:#64748B">Sheet: <code>${a.nama_sheet}</code> · ${a.total_rows} rows</div>
      </div>
      <div style="font-size:11px;color:#64748B">
        Buka Google Sheets<br>untuk lihat detail
      </div>
    </div>
  `).join('')}
  <div style="margin-top:12px;padding:10px;background:#FEF3C7;border-radius:8px;font-size:12px;color:#92400E">
    💡 <strong>Untuk melihat detail data arsip</strong>, buka Google Spreadsheet langsung.
    Sheet arsip tersedia per tahun dengan prefix <code>ABSENSI_ARSIP_</code>.
  </div>`;
}

// ============================================================
// PETA LOKASI ABSEN (v6.5)
// Pakai Leaflet + OpenStreetMap (gratis, no API key)
// ============================================================
async function bukaPetaLokasi() {
  if (typeof L === 'undefined') {
    showToast('Library peta belum siap. Refresh halaman dulu.', 'warning');
    return;
  }
  const tglInput = document.getElementById('peta-tgl')?.value;
  if (!tglInput) { showToast('Pilih tanggal dulu', 'warning'); return; }
  const tgl = fromInputDate(tglInput); // dd/MM/yyyy
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:16px;padding:0;width:100%;max-width:920px;
      height:88vh;display:flex;flex-direction:column;animation:fadeInScale .2s ease;overflow:hidden">
      <div style="padding:14px 20px;border-bottom:1px solid #E2E8F0;display:flex;
        ...
        <h3 style="font-size:16px;font-weight:700;margin:0">🗺️ Peta Lokasi Absen — ${tgl}</h3>
        <button onclick="document.getElementById('modal-peta').remove()"
          style="background:#F1F5F9;border:none;border-radius:50%;width:32px;height:32px;cursor:pointer">✕</button>
      </div>
      <div id="peta-stats" style="padding:8px 20px;background:#F8FAFC;font-size:12px;color:#64748B;border-bottom:1px solid #E2E8F0">
        <span class="spinner-btn" style="display:inline-block"></span> Memuat data...
      </div>
      <div id="peta-canvas" style="flex:1;background:#E2E8F0"></div>
    </div>`;
  document.body.appendChild(modal);

  try {
    // Ambil absensi tanggal tersebut
    const absensi = await callAPI('getAbsensiSemua', { tanggal: tgl });
    _renderPetaLokasi(absensi || [], tgl);
  } catch(e) {
    document.getElementById('peta-stats').innerHTML = `<span style="color:#EF4444">❌ ${e.message}</span>`;
  }
}

function _renderPetaLokasi(absensi, tgl) {
  // Filter yang punya koordinat valid
  const withGps = absensi.filter(a => {
    const lat = parseFloat(a.lat_masuk);
    const lon = parseFloat(a.lon_masuk);
    return !isNaN(lat) && !isNaN(lon) && lat !== 0 && lon !== 0;
  });

  const stat = document.getElementById('peta-stats');
  if (stat) {
    stat.innerHTML = `📊 <strong>${absensi.length}</strong> absensi tanggal ${tgl} ·
      <strong>${withGps.length}</strong> punya GPS valid ·
      <strong>${absensi.length - withGps.length}</strong> tanpa GPS`;
  }

  if (withGps.length === 0) {
    document.getElementById('peta-canvas').innerHTML = `
      <div style="display:flex;align-items:center;justify-content:center;height:100%;color:#94A3B8;font-size:14px">
        🗺️ Tidak ada data lokasi GPS untuk tanggal ini
      </div>`;
    return;
  }

  // Hitung center: rata-rata lat/lon
  const avgLat = withGps.reduce((s,a) => s + parseFloat(a.lat_masuk), 0) / withGps.length;
  const avgLon = withGps.reduce((s,a) => s + parseFloat(a.lon_masuk), 0) / withGps.length;

  // Init Leaflet map
  const map = L.map('peta-canvas').setView([avgLat, avgLon], 13);
  ...
  const STATUS_LABEL = {
    ...
    dinas_luar: '🚗 Dinas Luar', izin: '📝 Izin',
    sakit: '🏥 Sakit', cuti: '🏖️ Cuti', alfa: '❌ Alfa'
  };
  ...
  withGps.forEach(a => {
    ...
    // Custom colored marker (circle)
    const icon = L.divIcon({
      className: 'custom-marker',
      html: `<div style="background:${color};width:20px;height:20px;border-radius:50%;
        border:3px solid #fff;box-shadow:0 2px 6px rgba(0,0,0,0.4);cursor:pointer"></div>`,
      iconSize: [20, 20],
      iconAnchor: [10, 10]
    });

    const fotoUrl = a.foto_masuk_url
      ? (typeof normalizeDriveUrlFrontend === 'function'
          ? normalizeDriveUrlFrontend(a.foto_masuk_url)
          : a.foto_masuk_url)
      : '';

    const popupHtml = `
      <div style="min-width:200px;font-family:sans-serif">
        <div style="font-weight:700;font-size:14px;margin-bottom:4px">${a.nama_karyawan || a.id_karyawan}</div>
        <div style="display:inline-block;background:${color};color:#fff;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:600;margin-bottom:8px">
          ${label}
        </div>
        ${fotoUrl ? `<div style="margin-bottom:8px"><img src="${fotoUrl}" style="width:100%;max-width:180px;border-radius:6px;display:block" onerror="this.style.display='none'"></div>` : ''}
        <div style="font-size:12px;line-height:1.6;color:#475569">
          🕐 Masuk: <strong>${a.jam_masuk||'-'}</strong>${a.jam_keluar ? ' · Keluar: <strong>'+a.jam_keluar+'</strong>' : ''}<br>
          📍 ${lat.toFixed(6)}, ${lon.toFixed(6)}<br>
          ${a.jarak_meter_masuk ? '📏 ' + a.jarak_meter_masuk + 'm dari kantor<br>' : ''}
          ${a.keterangan ? '📝 <em>' + a.keterangan + '</em>' : ''}
        </div>
      </div>`;
  ...
  // Auto-fit bounds kalau lebih dari 1 marker
  if (withGps.length > 1) {
    const bounds = L.latLngBounds(withGps.map(a => [parseFloat(a.lat_masuk), parseFloat(a.lon_masuk)]));
    map.fitBounds(bounds, { padding: [40, 40], maxZoom: 16 });
  }

  // Force resize after modal animation (Leaflet fix)
