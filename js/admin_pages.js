    ...
    <div class="card" style="padding:12px;margin-bottom:12px">
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:8px">
        <input type="date" class="form-control" id="filter-tgl-abs"
          ...
          onchange="loadAbsensiAdminV4()">
        <select class="form-control" id="filter-status-abs" onchange="loadAbsensiAdminV4()">
        ...
        <select class="form-control" id="filter-dept-abs" onchange="loadAbsensiAdminV4()">
          <option value="">Semua Departemen</option>
        </select>
        <input type="text" class="form-control" id="filter-nama-abs"
          placeholder="Cari nama..." oninput="loadAbsensiAdminV4()">
        <button class="btn btn--secondary" style="font-size:13px" onclick="loadAbsensiAdminV4()">
          Tampilkan
        </button>
      </div>
    </div>
    ...
    <div class="card" style="padding:0;overflow-x:auto">
      <table class="simple-table" style="min-width:600px">
        <thead><tr><th>Karyawan</th><th>Jam Masuk</th><th>Jam Keluar</th>
          <th>Status</th><th>Jarak</th><th>Ket</th><th>Bukti</th><th>Aksi</th></tr></thead>
...
async function loadAbsensiAdminV4() {
  ...
  const fDept  = document.getElementById('filter-dept-abs')?.value||'';
  const fNama  = (document.getElementById('filter-nama-abs')?.value||'').toLowerCase().trim();
  const tbody  = document.getElementById('tbody-abs');
  if (!tbody||!tgl) return;
  tbody.innerHTML=`<tr><td colspan="8" style="padding:20px;text-align:center">${skeletonCard(1)}</td></tr>`;
  try {
    let data = await callAPI('getAbsensiSemua',{tanggal:fromInputDate(tgl)});
    // Filter departemen — ambil dari data karyawan
    if (!window._deptCache) {
      try {
        const kList = await callAPI('getKaryawanAktif',{});
        window._deptCache = {};
        (kList||[]).forEach(k=>{ window._deptCache[k.id_karyawan]=k.departemen||''; });
        // Isi dropdown departemen
        const deptEl = document.getElementById('filter-dept-abs');
        if (deptEl && deptEl.options.length <= 1) {
          const depts = [...new Set(Object.values(window._deptCache))].filter(Boolean).sort();
          depts.forEach(d=>{
            const opt = document.createElement('option');
            opt.value=d; opt.textContent=d;
            deptEl.appendChild(opt);
          });
          if (fDept) deptEl.value = fDept;
        }
      } catch(e) {}
    }
    if (fSt)   data=(data||[]).filter(a=>a.status===fSt);
    if (fDept) data=(data||[]).filter(a=>(window._deptCache?.[a.id_karyawan]||'')=== fDept);
    if (fNama) data=(data||[]).filter(a=>(a.nama_karyawan||'').toLowerCase().includes(fNama));
    ...
    tbody.innerHTML=data.map(a=>{
      // Cek apakah ada foto atau GPS
      const adaFotoMasuk = a.foto_masuk_url && a.foto_masuk_url.trim();
      const adaFotoKeluar= a.foto_keluar_url && a.foto_keluar_url.trim();
      const adaGPS       = (parseFloat(a.lat_masuk||0) !== 0) || (parseFloat(a.lon_masuk||0) !== 0);
      const adaBukti     = adaFotoMasuk || adaFotoKeluar || adaGPS;

      // Encode data untuk tombol bukti (hindari masalah quote)
      const buktiData = adaBukti ? encodeURIComponent(JSON.stringify({
        nama   : a.nama_karyawan||'',
        tanggal: a.tanggal||'',
        jam_masuk : a.jam_masuk||'',
        jam_keluar: a.jam_keluar||'',
        status    : a.status||'',
        jarak     : a.jarak_meter_masuk||0,
        foto_masuk : a.foto_masuk_url||'',
        foto_keluar: a.foto_keluar_url||'',
        lat_masuk  : a.lat_masuk||'',
        lon_masuk  : a.lon_masuk||'',
        lat_keluar : a.lat_keluar||'',
        lon_keluar : a.lon_keluar||'',
      ...
      <td style="font-size:12px;color:#64748B;max-width:80px;overflow:hidden;text-overflow:ellipsis">
        ${(a.keterangan||'-').substring(0,35)}</td>
      <td style="text-align:center">
        ${adaBukti
          ? `<button class="btn btn--ghost" style="padding:4px 8px;font-size:11px;color:#2D6CDF"
              onclick="lihatBuktiAbsenAdmin(decodeURIComponent('${buktiData}'))">
              ${adaFotoMasuk?'📸':'📍'}
            </button>`
...
// ─── LIHAT BUKTI ABSENSI (admin validasi foto + peta) ────────
function lihatBuktiAbsenAdmin(jsonStr) {
  let data;
  try { data = JSON.parse(jsonStr); } catch(e) { showToast('Data tidak valid','error'); return; }

  document.getElementById('modal-bukti-admin')?.remove();

  const normFotoUrl = url => {
    if (!url) return '';
  ...
  // mapId harus unik per peta (masuk vs keluar)
  const petaMini = (lat, lon, label, mapId) => {
  ...
  const jarakTxt = data.jarak > 0
    ? `<span style="color:#1A9E74;font-weight:600">±${data.jarak}m dari kantor</span>`
    : (data.status === 'dinas_luar' ? '<span style="color:#EA580C">Dinas Luar</span>' : '-');

  const modal = document.createElement('div');
  modal.id    = 'modal-bukti-admin';
  modal.style.cssText = `position:fixed;inset:0;background:rgba(0,0,0,.65);z-index:9900;
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:20px 20px 0 0;padding:20px;width:100%;
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
        <div>
          <div style="font-size:16px;font-weight:700">${data.nama}</div>
          <div style="font-size:12px;color:#64748B">
            ${formatTanggal(data.tanggal)} · ${badgeStatus(data.status)}
          </div>
        </div>
        <button onclick="document.getElementById('modal-bukti-admin').remove()"
      ...
      <div style="background:#F8FAFC;border-radius:10px;padding:12px;margin-bottom:16px;
        ...
        <div><span style="color:#94A3B8">Masuk</span><br><strong>${data.jam_masuk||'-'}</strong></div>
        <div><span style="color:#94A3B8">Keluar</span><br><strong>${data.jam_keluar||'-'}</strong></div>
        <div><span style="color:#94A3B8">Jarak</span><br>${jarakTxt}</div>
        <div><span style="color:#94A3B8">Ket.</span><br>
      ...
      <button class="btn btn--ghost btn--full" style="margin-top:8px"
        onclick="document.getElementById('modal-bukti-admin').remove()">Tutup</button>
    </div>`;
...
// ─── LEAFLET MAP HELPER ───────────────────────────────────────
function _initLeafletMaps(maps) {
  ...
  if (window.L) {
    ...
    script.onload  = ()=>setTimeout(doInit,150);
...
// ─── EDIT JAM MASUK / KELUAR ─────────────────────────────────
function modalEditJamAbsen(idAbsen, idKaryawan, namaKaryawan, tanggal, jamMasuk, jamKeluar, status) {
  document.getElementById('modal-edit-jam')?.remove();

  const LABEL = {
    hadir:'Hadir', terlambat:'Terlambat', alfa:'Alfa',
    izin:'Izin', sakit:'Sakit', cuti:'Cuti', dinas_luar:'Dinas Luar', libur:'Libur'
  };
  const warna = {
    hadir:'#1A9E74', terlambat:'#D97706', alfa:'#E53E3E',
    izin:'#2D6CDF', sakit:'#7C3AED', cuti:'#0891B2',
    dinas_luar:'#EA580C', libur:'#94A3B8'
  }[status] || '#64748B';

  const modal = document.createElement('div');
  modal.id = 'modal-edit-jam';
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:16px;padding:24px;
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
        <div>
          <div style="font-size:16px;font-weight:700">⏰ Edit Jam Absensi</div>
          <div style="font-size:12px;color:#64748B;margin-top:2px">
            ${namaKaryawan} · ${tanggal}
        ...
        <button onclick="document.getElementById('modal-edit-jam').remove()"
          style="background:#F1F5F9;border:none;border-radius:50%;width:32px;
          height:32px;font-size:16px;cursor:pointer;flex-shrink:0">✕</button>
      </div>

      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px">
        <div>
          <label style="font-size:12px;font-weight:600;color:#374151;
            display:block;margin-bottom:6px">Jam Masuk</label>
          <input type="time" id="input-jam-masuk" value="${jamMasuk}"
          ...
          <div style="font-size:10px;color:#94A3B8;margin-top:3px">
            Kosongkan jika belum absen
          </div>
        </div>
        <div>
          <label style="font-size:12px;font-weight:600;color:#374151;
            display:block;margin-bottom:6px">Jam Keluar</label>
          <input type="time" id="input-jam-keluar" value="${jamKeluar}"
            style="width:100%;padding:10px 12px;border:1.5px solid #E2E8F0;
            border-radius:10px;font-size:15px;font-family:monospace;
            outline:none;box-sizing:border-box"
            onfocus="this.style.borderColor='#2D6CDF'"
      ...
      <div style="background:#FEF3C7;border-radius:8px;padding:10px 12px;
        ...
        ⚠️ Perubahan jam akan langsung tersimpan ke data absensi.
      </div>

      <div style="display:flex;gap:10px">
        <button onclick="document.getElementById('modal-edit-jam').remove()"
          style="flex:1;padding:11px;border:1.5px solid #E2E8F0;border-radius:10px;
          background:#fff;color:#64748B;cursor:pointer;font-size:13px;font-weight:600">
          Batal
        </button>
        <button onclick="_simpanEditJamAbsen('${idAbsen}','${idKaryawan}','${tanggal}','${status}')"
          style="flex:2;padding:11px;border:none;border-radius:10px;
          background:linear-gradient(135deg,#2D6CDF,#1B5CBF);
...
async function _simpanEditJamAbsen(idAbsen, idKaryawan, tanggal, status) {
  // Ambil nilai jam dari input type="time" → format HH:MM
  const _getJam = (id) => {
    const v = (document.getElementById(id)?.value || '').trim();
    if (!v) return '';
    // Pastikan format HH:MM (ganti titik dengan titik dua kalau perlu)
  ...
  const jamMasuk  = _getJam('input-jam-masuk');
  const jamKeluar = _getJam('input-jam-keluar');

  // Validasi: jam masuk tidak boleh kosong kecuali alfa
  if (!jamMasuk && status !== 'alfa' && status !== 'izin' && status !== 'cuti'
      && status !== 'sakit' && status !== 'libur') {
    showToast('Jam masuk tidak boleh kosong untuk status ' + status, 'warning');
    return;
  }

  // Disable tombol simpan agar tidak double submit
  const btnSimpan = document.querySelector('#modal-edit-jam button:last-child');
  if (btnSimpan) {
  ...
  try {
    ...
    document.getElementById('modal-edit-jam')?.remove();
    showToast(r.message || 'Jam berhasil diubah', 'success');
    loadAbsensiAdminV4();
  } catch(e) {
...
async function tampilCetakAbsensiPDF() {
  const now    = new Date();
  const tanggal = document.getElementById('filter-tgl-abs')?.value || now.getFullYear()+'-'+String(now.getMonth()+1).padStart(2,'0')+'-'+String(now.getDate()).padStart(2,'0');
  const modal  = document.createElement('div');
      ...
      <div class="card" style="border-left:4px solid ${
        ...
        <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:10px">
          ...
          <div style="display:flex;flex-direction:column;gap:6px;flex-shrink:0">
            ${p.jenis==='izin'?`
            ${si?`
            <button class="btn" style="padding:9px 16px;font-size:13px;background:#1E5F3A;color:#fff;border:none;border-radius:8px;cursor:pointer;font-weight:600"
              onclick="_lihatSuratIzin('${si.id_surat}')">👁 Lihat Surat Izin</button>
            `:`
            <button class="btn" style="padding:9px 16px;font-size:13px;background:#1E5F3A;color:#fff;border:none;border-radius:8px;cursor:pointer;font-weight:600"
            ...
            <button class="btn btn--secondary" style="padding:9px 16px;font-size:13px"
              data-pgj="${p.id_pengajuan}"
              data-nama="${(p.nama_karyawan||'').replace(/"/g,'&quot;')}"
              data-jenis="${p.jenis}"
              data-id-surat="${p.jenis==='dinas_luar'&&stAdm?stAdm.id_surat:p.jenis==='izin'&&si?si.id_surat:''}"
              data-st-status="${p.jenis==='dinas_luar'?stAdmStatus:p.jenis==='izin'?siStatus:''}"
              onclick="_setujuiPengajuanClick(this)">
              ✅ Setujui</button>
            `:''}
...
async function approvePGJ(id,nama) {
...
async function tolakPGJ(id,nama) {
...
async function approveLemburAdmin(id){showModal('✅ Setujui Lembur?','',async()=>{
  const r=await callAPI('approveLembur',{id_lembur:id});showToast(r.message,'success');loadLemburAdmin('pending');
},'✅ Setujui');}
async function tolakLemburAdmin(id){showModal('❌ Tolak Lembur?','',async()=>{
...
async function tampilKwitansiRentang() {
  ...
  const y2  = now.getFullYear()+'-'+String(now.getMonth()+1).padStart(2,'0')+'-'+String(now.getDate()).padStart(2,'0');
...
async function hapusPengumumanAdmin(id){
...
async function hapusLokasiAdmin(id,nama){
...
// ─── EDIT JADWAL MANUAL ──────────────────────────────────────
function editJadwalManual(idKaryawan, tanggal, kodeSekarang, namaKaryawan) {
  idKaryawan   = decodeURIComponent(idKaryawan);
  tanggal      = decodeURIComponent(tanggal);
  namaKaryawan = decodeURIComponent(namaKaryawan);

  document.getElementById('modal-edit-jadwal')?.remove();

  const KC = {
    'P': { label:'Pagi',  color:'#1A9E74', desc:'Shift Pagi' },
    'S': { label:'Sore',  color:'#D97706', desc:'Shift Sore' },
    'M': { label:'Malam', color:'#6C63FF', desc:'Shift Malam' },
    'L': { label:'Libur', color:'#94A3B8', desc:'Hari Libur' },
  };

  const p = tanggal.split('/');
  const tglFmt = p[0]+'/'+p[1]+'/'+p[2];
  const hariIdx = new Date(+p[2],+p[1]-1,+p[0]).getDay();
  const hariNama = ['Minggu','Senin','Selasa','Rabu','Kamis','Jumat','Sabtu'][hariIdx];

  const modal = document.createElement('div');
  modal.id    = 'modal-edit-jadwal';
  modal.style.cssText = `position:fixed;inset:0;background:rgba(0,0,0,.5);
    z-index:9500;display:flex;align-items:center;justify-content:center;
    padding:20px;backdrop-filter:blur(4px)`;
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:16px;padding:24px;
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
        <div>
          <div style="font-size:16px;font-weight:700">✏️ Edit Jadwal Manual</div>
          <div style="font-size:12px;color:#64748B;margin-top:2px">
            ${namaKaryawan} · ${hariNama}, ${tglFmt}
          </div>
        </div>
        <button onclick="document.getElementById('modal-edit-jadwal').remove()"
          style="background:#F1F5F9;border:none;border-radius:50%;width:32px;height:32px;
          font-size:16px;cursor:pointer">✕</button>
      </div>
      ...
      <div style="font-size:12px;color:#64748B;margin-bottom:10px">
        Jadwal sekarang:
        <strong style="color:${KC[kodeSekarang]?.color||'#64748B'}">
          ${KC[kodeSekarang]?.label||kodeSekarang}
        </strong>
      </div>

      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:20px">
        ${Object.entries(KC).map(([k,v])=>`
          <button onclick="_simpanJadwalManual('${idKaryawan}','${tanggal}','${k}')"
            style="padding:14px 8px;border-radius:12px;border:2px solid ${k===kodeSekarang?v.color:'#E2E8F0'};
              background:${k===kodeSekarang?v.color+'22':'#fff'};
              color:${v.color};cursor:pointer;font-weight:700;font-size:13px;
              transition:all .15s;text-align:center"
            onmouseover="this.style.borderColor='${v.color}';this.style.background='${v.color}22'"
            onmouseout="${k!==kodeSekarang?"this.style.borderColor='#E2E8F0';this.style.background='#fff'":""}">
      ...
      <div style="background:#FEF3C7;border-radius:8px;padding:10px 12px;
        ...
        ⚠️ Perubahan jadwal manual akan menimpa jadwal otomatis.
        Catat alasan perubahan jika diperlukan.
      </div>
    </div>`;
...
async function _simpanJadwalManual(idKaryawan, tanggal, kode) {
  try {
    const r = await callAPI('editJadwalManual', {
      id_karyawan : idKaryawan,
      tanggal     : tanggal,
      kode        : kode
    });
    document.getElementById('modal-edit-jadwal')?.remove();
    showToast(r.message || 'Jadwal berhasil diubah', 'success');
    loadJadwalAdmin(); // refresh tabel
  } catch(e) {
    showToast(e.message, 'error');
  }
}
            ...
            ${karyawanIds.map(id=>{
              ...
              return `<tr>
                ...
                ${tanggals.map(t=>{
                  ...
                  const _t=encodeURIComponent(t);
                  const _nm=encodeURIComponent(k.nama||'');
                  return `<td style="text-align:center;padding:3px 2px;border-bottom:1px solid #F1F5F9">
                    <button
                      onclick="editJadwalManual('${_id}','${_t}','${kode}','${_nm}')"
                      title="Klik untuk ubah jadwal"
                      style="background:${c}22;color:${c};border:1px solid ${c}55;
                        padding:3px 7px;border-radius:6px;font-weight:700;font-size:10px;
                        min-width:26px;cursor:pointer;transition:transform .1s"
...
async function resetDeviceAdmin(id,nama){
...
// Handler terpusat untuk tombol Setujui di card pengajuan
// Menentukan fungsi yang dipanggil berdasarkan jenis & status surat
async function _setujuiPengajuanClick(btn) {
  const idPgj    = btn.dataset.pgj    || '';
  const nama     = btn.dataset.nama   || '';
  const jenis    = btn.dataset.jenis  || '';
  const idSurat  = btn.dataset.idSurat || '';
  const stStatus = btn.dataset.stStatus || '';

  // Dinas luar — semua TTD selesai → setujui via surat tugas
  if (jenis === 'dinas_luar' && idSurat && stStatus === 'menunggu_admin') {
    return _setujuiSuratTugasAdmin(idSurat);
  }
  // Izin — semua TTD selesai → setujui via surat izin
  if (jenis === 'izin' && idSurat && stStatus === 'menunggu_admin') {
    return _setujuiSuratIzinAdmin(idSurat);
  }
  // Sakit / Cuti / Perpanjangan — approve pengajuan langsung
  approvePGJ(idPgj, nama);
}
...
async function _setujuiSuratIzinAdmin(idSurat) {
...
async function unlockAkunAdmin(username, nama) {
  showModal('🔓 Buka Kunci Akun?',
    `Akun <strong>${nama}</strong> (${username}) dikunci karena terlalu banyak percobaan login gagal. Buka kuncinya?`,
    async () => {
      const r = await callAPI('unlockAkun', { username });
...
// ─────────────────────────────────────────────────────────────
async function _refreshHariLibur(tahun) {
  const el = document.getElementById('hari-libur-info');
  if (el) el.innerHTML = '<span style="color:#94A3B8;font-size:12px">Memuat...</span>';
  try {
    const r = await callAPI('refreshHariLibur', { tahun });
    if (el) el.innerHTML = `<div style="font-size:12px;color:${r.success?'#1A9E74':'#D97706'};
      margin-top:6px">${r.message}</div>`;
  } catch(e) {
    if (el) el.innerHTML = `<div style="font-size:12px;color:#E53E3E">${e.message}</div>`;
  }
}

async function _lihatHariLibur() {
  try {
    const tahun = new Date().getFullYear();
    const data  = await callAPI('getHariLibur', { tahun });
    const el    = document.getElementById('hari-libur-info');
    if (!el) return;
    if (!data || !data.length) { el.innerHTML = '<span style="font-size:12px;color:#94A3B8">Belum ada data</span>'; return; }
    el.innerHTML = `<div style="margin-top:10px;max-height:200px;overflow-y:auto;font-size:12px">
      ${data.map(h=>`<div style="display:flex;justify-content:space-between;
        padding:4px 0;border-bottom:1px solid #F1F5F9">
        <span>${h.tanggal}</span>
        <span style="color:#64748B">${h.nama}</span>
      </div>`).join('')}
    </div>`;
  } catch(e) { showToast(e.message,'error'); }
}
    ...
    el.innerHTML=`
      ...
      <div class="card">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
          ...
          ${_si('periode_mulai_tgl','Periode Mulai Tanggal (1=bulan kalender, 25=tgl 25 s/d 24)',map,'number')}
...
${map['ucapan_ulang_tahun_template']||''}</textarea>
        ...
        <div class="form-group">
          ...
          <select class="form-control" id="set-aktif_email_notif">
            <option value="true" ${String(map['aktif_email_notif']).toLowerCase()==='true'?'selected':''}>
              ✅ Aktif — Kirim email otomatis ke karyawan & admin</option>
            <option value="false" ${String(map['aktif_email_notif']).toLowerCase()==='false'?'selected':''}>
              ❌ Nonaktif — Tidak ada email dikirim</option>
          </select>
          <p class="form-hint">Pastikan kolom email karyawan sudah terisi & GAS sudah diotorisasi MailApp</p>
        </div>
      </div>

      <!-- Notifikasi Email — aksi lanjutan -->
      <div class="card">
        <h3 style="font-size:14px;font-weight:700;color:#64748B;text-transform:uppercase;
          letter-spacing:.6px;margin-bottom:8px">📧 Notifikasi Email</h3>
        <div style="background:#F0F9FF;border-radius:8px;padding:12px;font-size:12px;
          color:#0369A1;margin-bottom:14px;line-height:1.8">
          <strong>Email dikirim otomatis untuk:</strong><br>
          📨 Pengajuan baru masuk → Admin<br>
          ✅ Pengajuan disetujui → Karyawan<br>
          ❌ Pengajuan ditolak → Karyawan<br>
          ⏰ Belum absen keluar jam 18.00 → Karyawan (butuh setup trigger)
        </div>
        <div style="display:flex;gap:10px;flex-wrap:wrap">
          <button onclick="_setupEmailTrigger()"
            style="flex:1;padding:10px;background:#0369A1;color:#fff;border:none;
            border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;min-width:140px">
            ⏰ Setup Trigger Reminder
          </button>
          <button onclick="_testKirimEmail()"
        ...
        <div id="email-notif-result" style="margin-top:10px;font-size:12px"></div>
      </div>
      ...
      <div class="card" style="border-left:4px solid #0369A1;margin-top:4px">
        <h3 style="font-size:14px;font-weight:700;color:#0369A1;text-transform:uppercase;
          letter-spacing:.6px;margin-bottom:8px">🗄️ Archive Absensi Otomatis</h3>
        <p style="font-size:12px;color:#64748B;margin-bottom:12px;line-height:1.6">
          Data absensi lebih dari <strong>12 bulan</strong> dipindah otomatis ke sheet
          <strong>ABSENSI_ARSIP_YYYY</strong> setiap tanggal 1. Menjaga performa sistem tetap cepat.
        </p>
        <div id="archive-trigger-info" style="font-size:12px;color:#0369A1;margin-bottom:10px"></div>
        <div style="display:flex;gap:8px;flex-wrap:wrap">
          <button onclick="_setupArchiveTrigger()"
            style="flex:1;padding:10px;background:#0369A1;color:#fff;border:none;
            border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;min-width:140px">
            ⚙️ Aktifkan Trigger
          </button>
          <button onclick="_testArchiveSekarang()"
            style="flex:1;padding:10px;background:#F0F9FF;color:#0369A1;
            border:1.5px solid #BAE6FD;border-radius:8px;font-size:13px;
            font-weight:600;cursor:pointer;min-width:140px">
            🧪 Test Archive Sekarang
          </button>
          <button onclick="_stopArchiveTrigger()"
            style="flex:1;padding:10px;background:#FEF2F2;color:#DC2626;
      ...
      <div class="card" style="border-left:4px solid #E53E3E;margin-top:4px">
        <h3 style="font-size:14px;font-weight:700;color:#C53030;text-transform:uppercase;
          letter-spacing:.6px;margin-bottom:8px">🗑️ Log Aktivitas</h3>
        <p style="font-size:12px;color:#64748B;margin-bottom:12px;line-height:1.6">
          Kosongkan seluruh isi sheet LOG_AKTIVITAS. Header tetap, 1 baris jejak pembersihan
          akan dicatat otomatis. Tindakan ini <strong>tidak bisa dibatalkan</strong>.
        </p>
        <button onclick="clearLogAktivitasAdmin()"
          style="width:100%;padding:10px;background:#FFF5F5;color:#C53030;
          border:1.5px solid #FC8181;border-radius:8px;font-size:13px;
          font-weight:600;cursor:pointer">
          🗑️ Kosongkan Log Aktivitas
        </button>
      </div>` : ''}
...
async function _setupArchiveTrigger() {
  ...
  try {
    const res = await callAPI('setupArchiveTrigger', {});
    showToast('✅ Trigger archive aktif!', 'success', 5000);
    if (el) el.innerHTML = '<span style="color:#15803D">✅ Trigger aktif — berjalan setiap tanggal 1 jam 03.00</span>';
  } catch(e) {
    showToast('Gagal: ' + e.message, 'error');
...
async function _stopArchiveTrigger() {
  if (!confirm('Hentikan trigger archive otomatis?')) return;
  const el = document.getElementById('archive-result');
  try {
    const res = await callAPI('stopArchiveTrigger', {});
    showToast('Trigger archive dihentikan', 'info', 3000);
    if (el) el.innerHTML = '<span style="color:#64748B">⏹ Trigger dihentikan</span>';
  } catch(e) {
    showToast('Gagal: ' + e.message, 'error');
  }
}

async function _testArchiveSekarang() {
  if (!confirm('Jalankan archive sekarang? Data absensi >12 bulan akan dipindah ke sheet ABSENSI_ARSIP_YYYY.')) return;
  const el = document.getElementById('archive-result');
  if (el) el.innerHTML = '<span style="color:#0369A1">Menjalankan archive...</span>';
  try {
    showToast('Menjalankan archive, harap tunggu...', 'info', 5000);
    const res = await callAPI('testArchiveSekarang', {});
    const msg = '✅ Archive selesai — ' + (res.total_archived||0) + ' baris diarsip, ' +
      (res.total_kept||0) + ' baris tetap aktif';
    showToast(msg, 'success', 6000);
    if (el) el.innerHTML = '<span style="color:#15803D">' + msg + '</span>';
  } catch(e) {
    showToast('Gagal: ' + e.message, 'error');
    if (el) el.innerHTML = '<span style="color:#E53E3E">❌ ' + e.message + '</span>';
  }
...
async function _setupEmailTrigger() {
  const el = document.getElementById('email-notif-result');
  if (el) el.innerHTML = '<span style="color:#0369A1">Mengatur trigger...</span>';
  try {
    const res = await callAPI('setupEmailTrigger', {});
...
async function _testKirimEmail() {
  ...
  try {
    const res = await callAPI('testKirimEmail', {});
    showToast('✅ ' + res.message, 'success', 5000);
    if (el) el.innerHTML = '<span style="color:#15803D">✅ ' + res.message + '</span>';
  } catch(e) {
    showToast('Gagal: ' + e.message, 'error');
...
async function clearLogAktivitasAdmin() {
  const konfirm = prompt('Ketik HAPUS untuk mengosongkan seluruh log aktivitas:');
  if (konfirm !== 'HAPUS') { showToast('Dibatalkan', 'info'); return; }
  try {
    showToast('Mengosongkan log...', 'info', 2000);
    const res = await callAPI('clearLogAktivitas', {});
    showToast('✅ ' + res.message, 'success', 4000);
  } catch(e) { showToast('Gagal: ' + e.message, 'error'); }
}
...
async function tampilFormHargaLembur(data){
...
function _filterRekapDL() {
  ...
  // Simpan data filtered untuk export
  window._rdlDataFiltered = filtered;
...
async function _exportRekapDL() {
  const filtered = _rdlDataFiltered || _rdlData;
  ...
  try {
    ...
    // Kirim data ke backend GAS untuk generate Excel profesional
    const res = await callAPI('exportRekapDinasLuar', { data: filtered });
    ...
    const tgl  = new Date().toLocaleDateString('id').replace(/\//g,'-');
    a.href     = url;
    a.download = `Rekap_Dinas_Luar_${tgl}.xlsx`;
