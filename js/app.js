...
async function _exportFotoPDF() {
  ...
  try {
    ...
    let instansi = {};
    try {
      const settArr = await callAPI('getAllSetting', {});  // route: getAllSetting (tanpa s)
    ...
    <div class="admin-layout">
      <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar__nav" id="sidebar-nav">
          ...
          ${_si2('surat-tugas-admin','📋','Surat Tugas')}
...
function routeAdmin(page) {
  ...
  'surat-tugas-admin': ['Surat Tugas',      ()=>loadSuratTugasAdmin()],
    ...
    <div style="display:flex;flex-direction:column;gap:14px">
      <div>
        <label style="font-size:12px;font-weight:600;color:#64748B;display:block;margin-bottom:6px">
          TUJUAN TUGAS</label>
        <input id="st-tujuan" type="text" placeholder="Contoh: Sukabumi, Jawa Barat"
          value="${keterangan.split('@')[1]?.split(',')[0]?.trim()||''}"
          style="width:100%;padding:10px;border:1px solid #E2E8F0;border-radius:8px;
          font-size:13px;box-sizing:border-box">
      </div>
      <div>
...
// ── PERPANJANG DINAS LUAR (karyawan) ─────────────────────────
async function _perpanjangDinasLuar(idSurat) {
  ...
  <div style="background:#fff;border-radius:16px;padding:24px;
    ...
    <div style="font-size:13px;color:#64748B;margin-bottom:16px">
      Ajukan perpanjangan periode dinas luar</div>
    ...
    <div style="display:flex;flex-direction:column;gap:12px">
      ...
      <div>
        ...
        <textarea id="perp-alasan" rows="3" placeholder="Jelaskan alasan perpanjangan..."
          style="width:100%;padding:10px;border:1px solid #E2E8F0;border-radius:8px;
          font-size:13px;box-sizing:border-box;resize:vertical"></textarea>
      </div>
    </div>
...
async function _kirimPerpanjangan(idSurat) {
  ...
  try {
    showToast('Mengirim pengajuan perpanjangan...', 'info', 2000);
    await callAPI('perpanjangDinasLuar', {
      id_surat          : idSurat,
      tanggal_selesai_baru: toIDDate(tglBaru),
      alasan            : alasan
    });
    document.getElementById('modal-perpanjang-dl')?.remove();
    showToast('✅ Perpanjangan berhasil diajukan ke admin', 'success', 4000);
  } catch(e) {
    showToast('Gagal: '+e.message, 'error');
  }
}
