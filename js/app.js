  ...
  container.innerHTML = `
    <div class="page-content">
      ...
      <div class="absen-btn-grid">
        <button class="absen-btn absen-btn--masuk" id="btn-absen-masuk" onclick="mulaiAbsenMasukV4()">
        ...
        <button class="absen-btn absen-btn--keluar" id="btn-absen-keluar" onclick="mulaiAbsenKeluarV4()">
          <span class="absen-btn__icon">🏁</span>
  ...
  container.innerHTML = `
    <div class="page-content">
      ...
      <div style="display:flex;gap:8px;margin-bottom:12px">
        <select class="form-control" id="filter-bulan-absensi" style="flex:1"
          onchange="loadHalamanAbsensi()">
          ${Array.from({length:12},(_,i)=>`<option value="${i+1}"
            ${i===now.getMonth()?'selected':''}>${bulanNama(i+1)}</option>`).join('')}
        </select>
        <select class="form-control" id="filter-tahun-absensi" style="width:90px"
          onchange="loadHalamanAbsensi()">
    ...
    <div class="admin-layout">
      ...
      <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar__nav" id="sidebar-nav">
          ...
          ${sidebarItem('harga-lembur-admin','💰','Harga Lembur')}
...
function toggleSidebar() {
  ...
  if (!sidebar) return;
  // Mobile: gunakan class 'open' + backdrop
...
function routeAdmin(page) {
  ...
  const pages = {
    'dashboard-admin' : ['Dashboard',           c => { c.innerHTML=`<div id="admin-stats-container">${skeletonCard(4)}</div><div id="ultah-section" style="display:none;margin-top:16px"></div><div class="card" style="margin-top:16px"><h3 style="font-size:15px;font-weight:700;margin-bottom:16px">📊 Kehadiran 6 Bulan Terakhir</h3><canvas id="chart-6bulan" style="max-height:280px"></canvas></div><div id="admin-ranking-section" style="margin-top:16px"></div>`; loadDashboardAdminV3(); }],
    'absensi-semua'   : ['Absensi Karyawan',    c => renderAbsensiAdminFull(c)],
    'karyawan-list'   : ['Manajemen Karyawan',  renderKaryawanAdminFull],
    'pengajuan-admin' : ['Pengajuan',            c => renderPengajuanAdminFull(c)],
    'lembur-admin'    : ['Lembur',               c => renderLemburAdminFull(c)],
    'ranking-admin'   : ['Ranking Karyawan',     renderRankingAdminFull],
    'sp-admin'        : ['Surat Peringatan',     renderSPAdminFull],
    'pengumuman-admin': ['Pengumuman',           renderPengumumanAdminFull],
    'lokasi-admin'    : ['Lokasi Kantor',        renderLokasiAdminV3],
    'shift-admin'     : ['Shift & Jadwal',       renderShiftJadwalAdmin],
    'device-admin'    : ['Reset Device',         renderDeviceAdmin],
    'pengaturan-admin': ['Pengaturan Sistem',    c => { c.innerHTML=`<h2 style="font-size:17px;font-weight:700;margin-bottom:16px">⚙️ Pengaturan Sistem</h2><div id="pengaturan-container">${skeletonCard(4)}</div>`; loadPengaturanAdminV3(); }],
    'laporan-admin'   : ['Laporan & Rekap',      renderLaporanAdminV3],
    'harga-lembur-admin': ['Harga Lembur',       renderHargaLemburAdmin]
  };
