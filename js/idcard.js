...
async function renderIDCardToCanvas(canvas, k, instansi, opts) {
  ...
  const HEADER_H  = Math.round(H * 0.28);  // 28% header biru (optimal)
  const FOOTER_H  = Math.round(H * 0.10);  // 10% footer biru
  ...
  const FOTO_CY  = HEADER_H - Math.round(FOTO_SZ * 0.35); // overlap 35%
  ...
  if (includeQR) {
    try {
      ...
      const QR_SZ  = Math.max(s(380), Math.min(s(420), Math.floor(spaceLeft * 0.90)));
      const QR_PAD = s(16);
