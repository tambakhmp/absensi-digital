...
async function renderIDCardToCanvas(canvas, k, instansi, opts) {
  ...
  // Layout zones
  const HEADER_H = Math.round(H * 0.26);
  ...
  // ── LOAD SEMUA ASSET DULU ────────────────────────────────
  const logoUrl = instansi?.logo_url
    ? (typeof normalizeDriveUrlFrontend==='function'
        ? normalizeDriveUrlFrontend(instansi.logo_url) : instansi.logo_url) : '';
  const fotoUrl = k.foto_profil_url
    ? (typeof normalizeDriveUrlFrontend==='function'
        ? normalizeDriveUrlFrontend(k.foto_profil_url) : k.foto_profil_url) : '';
  const bgUrl   = instansi?.bg_idcard_url || instansi?.foto_kantor_url || logoUrl;

  let logoImg = null, fotoImg = null, bgImg = null;
  const loadP = [
    logoUrl ? _loadImage(logoUrl).catch(()=>null) : Promise.resolve(null),
    fotoUrl ? _loadImage(fotoUrl).catch(()=>null) : Promise.resolve(null),
    bgUrl   ? _loadImage(bgUrl).catch(()=>null)   : Promise.resolve(null),
  ];
  [logoImg, fotoImg, bgImg] = await Promise.all(loadP);

  // ── WATERMARK LATAR BELAKANG (logo/foto kantor) ──────────
  if (bgImg) {
    ctx.save();
    ctx.globalAlpha = 0.055; // sangat transparan, hanya kesan latar
    const bgSz  = Math.round(W * 0.75);
    const bgX   = (W - bgSz) / 2;
    const bgY   = HEADER_H + Math.round((FOOTER_Y - HEADER_H - bgSz) / 2);
    // Crop persegi tengah
    const sMin  = Math.min(bgImg.width, bgImg.height);
    const sX    = (bgImg.width  - sMin) / 2;
    const sY    = (bgImg.height - sMin) / 2;
    ctx.drawImage(bgImg, sX, sY, sMin, sMin, bgX, bgY, bgSz, bgSz);
    ctx.restore();
  }

  // ── HEADER BIRU ───────────────────────────────────────────
  const hGrad = ctx.createLinearGradient(0, 0, W*0.4, HEADER_H);
  hGrad.addColorStop(0, '#0A2F6B');
  hGrad.addColorStop(0.55, '#1B5CBF');
  hGrad.addColorStop(1, '#2D6CDF');
  ctx.fillStyle = hGrad;
  ...
  for (let i = -H; i < W+H; i += s(22)) {
    ...
    ctx.strokeStyle='#FFFFFF'; ctx.lineWidth=s(9); ctx.stroke();
  }
  ctx.restore();

  // Gold strip
  const gs = ctx.createLinearGradient(0,0,W,0);
  gs.addColorStop(0,'#B8860B'); gs.addColorStop(0.3,'#F0C040');
  gs.addColorStop(0.7,'#D4A017'); gs.addColorStop(1,'#B8860B');
  ctx.fillStyle = gs;
  ...
  if (logoImg) {
    ...
    ctx.save();
    ctx.beginPath();
    ctx.arc(cx, cY+lSz/2, lSz/2+s(9), 0, Math.PI*2);
    ...
    ctx.save();
    ctx.beginPath();
    ctx.arc(cx, cY+s(40), s(40), 0, Math.PI*2);
    ctx.fillStyle='rgba(255,255,255,0.18)'; ctx.fill();
    ctx.fillStyle=CLR.white;
    ctx.font=`bold ${s(28)}px Arial`;
    ctx.textAlign='center'; ctx.textBaseline='middle';
    ctx.fillText(namaInst.charAt(0), cx, cY+s(40));
    ctx.restore();
    cY += s(80)+s(12);
  }
  ...
  while (ctx.measureText(namaInst).width > W-s(48) && nInstSz > s(18)) {
    nInstSz -= 2;
    ctx.font=`900 ${nInstSz}px Arial, sans-serif`;
  }
  const instLines = _wrapText(ctx, namaInst, W-s(48), nInstSz);
  ctx.save();
  ctx.shadowColor='rgba(0,0,0,0.25)'; ctx.shadowBlur=s(4); ctx.shadowOffsetY=s(2);
  instLines.forEach((ln,i) => ctx.fillText(ln, cx, cY + i*nInstSz*1.25));
  ctx.restore();
  cY += instLines.length * nInstSz * 1.25 + s(10);

  // Garis gold tipis (PT HMP dihapus)
  const gg = ctx.createLinearGradient(W*0.2,0,W*0.8,0);
  gg.addColorStop(0,'transparent'); gg.addColorStop(0.3,CLR.gold3);
  gg.addColorStop(0.7,CLR.gold2); gg.addColorStop(1,'transparent');
  ctx.strokeStyle=gg; ctx.lineWidth=s(1.5);
  ctx.beginPath(); ctx.moveTo(W*0.2,cY); ctx.lineTo(W*0.8,cY); ctx.stroke();
  // PT HMP DIHAPUS sesuai permintaan

  // ── FOTO KARYAWAN (40% lebar, overlap 30%) ───────────────
  const FOTO_SZ = Math.round(W * 0.40);
  const FOTO_CY = HEADER_H - Math.round(FOTO_SZ * 0.30);
  const FOTO_X  = cx - FOTO_SZ/2;

  // Shadow
  ctx.save();
  ctx.shadowColor='rgba(13,59,122,0.25)'; ctx.shadowBlur=s(22); ctx.shadowOffsetY=s(8);
  ctx.beginPath(); ctx.arc(cx, FOTO_CY+FOTO_SZ/2, FOTO_SZ/2+s(8), 0, Math.PI*2);
  ctx.fillStyle=CLR.white; ctx.fill();
  ctx.restore();

  // Border gold
  const gb = ctx.createLinearGradient(FOTO_X,FOTO_CY,FOTO_X+FOTO_SZ,FOTO_CY+FOTO_SZ);
  gb.addColorStop(0,CLR.gold3); gb.addColorStop(0.5,CLR.gold1); gb.addColorStop(1,CLR.gold3);
  ctx.save();
  ctx.beginPath(); ctx.arc(cx, FOTO_CY+FOTO_SZ/2, FOTO_SZ/2+s(8), 0, Math.PI*2);
  ctx.strokeStyle=gb; ctx.lineWidth=s(5); ctx.stroke();
  ctx.beginPath(); ctx.arc(cx, FOTO_CY+FOTO_SZ/2, FOTO_SZ/2+s(13), 0, Math.PI*2);
  ctx.strokeStyle='rgba(212,160,23,0.28)'; ctx.lineWidth=s(2); ctx.stroke();
  ctx.restore();

  // Clip foto
  ctx.save();
  ctx.beginPath(); ctx.arc(cx, FOTO_CY+FOTO_SZ/2, FOTO_SZ/2, 0, Math.PI*2); ctx.clip();
  if (fotoImg) {
    ...
    ctx.fillStyle='#CBD5E1'; ctx.fillRect(FOTO_X,FOTO_CY,FOTO_SZ,FOTO_SZ);
    ctx.fillStyle=CLR.white; ctx.font=`bold ${s(70)}px Arial`;
    ctx.textAlign='center'; ctx.textBaseline='middle';
  ...
  while (ctx.measureText(nama).width > W-s(52) && nFsz > s(26)) {
    nFsz-=2; ctx.font=`900 ${nFsz}px Arial, sans-serif`;
  }
  const namaLines = _wrapText(ctx, nama, W-s(52), nFsz);
  ctx.save();
  ctx.shadowColor='rgba(0,0,0,0.07)'; ctx.shadowBlur=s(3); ctx.shadowOffsetY=s(2);
  namaLines.forEach((ln,i) => ctx.fillText(ln, cx, tY+i*nFsz*1.2));
  ctx.restore();
  tY += namaLines.length * nFsz * 1.2 + s(8);

  // JABATAN badge
  let jFsz = s(26);
  ctx.font=`800 ${jFsz}px Arial, sans-serif`;
  while (ctx.measureText(jabatan).width > W-s(80) && jFsz > s(15)) {
    jFsz-=2; ctx.font=`800 ${jFsz}px Arial, sans-serif`;
  }
  const jabW = ctx.measureText(jabatan).width + s(40);
  const jabH = jFsz + s(14);
  const jabX = cx - jabW/2;
  ctx.save();
  const jabGr = ctx.createLinearGradient(jabX,tY,jabX+jabW,tY+jabH);
  jabGr.addColorStop(0,'rgba(29,91,213,0.10)');
  jabGr.addColorStop(1,'rgba(45,108,223,0.06)');
  ctx.fillStyle=jabGr;
  _roundRect(ctx, jabX, tY-s(2), jabW, jabH, s(20)); ctx.fill();
  ctx.strokeStyle='rgba(45,108,223,0.22)'; ctx.lineWidth=s(1.5);
  _roundRect(ctx, jabX, tY-s(2), jabW, jabH, s(20)); ctx.stroke();
  ctx.restore();
  ...
  ctx.font=`700 ${s(19)}px Arial, sans-serif`;
  ctx.fillText(dept, cx, tY);
  tY += s(20);

  // Divider gold
  const dg = ctx.createLinearGradient(W*0.1,0,W*0.9,0);
  dg.addColorStop(0,'transparent'); dg.addColorStop(0.3,CLR.gold2);
  dg.addColorStop(0.7,CLR.gold2); dg.addColorStop(1,'transparent');
  ctx.strokeStyle=dg; ctx.lineWidth=s(1.5);
  ...
  ctx.save();
  ctx.shadowColor='rgba(29,91,213,0.07)'; ctx.shadowBlur=s(10); ctx.shadowOffsetY=s(3);
  ctx.fillStyle='#EBF2FF';
  _roundRect(ctx, BOX_X, BOX_Y, BOX_W, BOX_H, s(10)); ctx.fill();
  ctx.restore();
  ctx.strokeStyle='#C5D9F8'; ctx.lineWidth=s(1.5);
  _roundRect(ctx, BOX_X, BOX_Y, BOX_W, BOX_H, s(10)); ctx.stroke();
  ctx.strokeStyle='#C5D9F8'; ctx.lineWidth=s(1);
  ctx.beginPath();
  ctx.moveTo(BOX_X+BOX_W/2, BOX_Y+s(10));
  ctx.lineTo(BOX_X+BOX_W/2, BOX_Y+BOX_H-s(10)); ctx.stroke();

  const nikLY  = BOX_Y + BOX_H*0.30;
  const nikVY  = BOX_Y + BOX_H*0.68;
  const nikCX  = BOX_X + BOX_W*0.25;
  const tglCX  = BOX_X + BOX_W*0.75;

  ctx.textAlign='center'; ctx.textBaseline='middle';
  ctx.fillStyle=CLR.gray2;
  ctx.font=`700 ${s(14)}px Arial, sans-serif`;
  ctx.fillText('N I K', nikCX, nikLY);
  ctx.fillText('TGL MASUK', tglCX, nikLY);

  ctx.fillStyle=CLR.dark;
  let nikSz = s(22);
  ctx.font=`900 ${nikSz}px Arial, sans-serif`;
  while(ctx.measureText(nik).width > BOX_W/2-s(18) && nikSz>s(13)){
    nikSz--; ctx.font=`900 ${nikSz}px Arial, sans-serif`;
  }
  ctx.fillText(nik, nikCX, nikVY);

  let tglSz = s(19);
  ctx.font=`900 ${tglSz}px Arial, sans-serif`;
  while(ctx.measureText(tglMasuk).width > BOX_W/2-s(14) && tglSz>s(12)){
    tglSz--; ctx.font=`900 ${tglSz}px Arial, sans-serif`;
  }
  ctx.fillText(tglMasuk, tglCX, nikVY);
  ...
  if (includeQR) {
    try {
      const spaceLeft = FOOTER_Y - s(16) - tY;
      const QR_SZ     = Math.max(s(360), Math.min(s(400), Math.floor(spaceLeft * 0.86)));
      const QR_PAD    = s(14);
      const QR_BOX    = QR_SZ + QR_PAD*2;
      const QR_Y      = tY + s(6);
      const QR_X      = cx - QR_SZ/2;
      const BX        = QR_X - QR_PAD;
      const BY        = QR_Y;

      const qrDataUrl = await _generateQRDataURL(k.id_karyawan, QR_SZ + s(40));
      if (qrDataUrl) {
        const qrImg = await _loadImage(qrDataUrl);

        // Shadow QR box
        ctx.save();
        ctx.shadowColor='rgba(13,59,122,0.15)'; ctx.shadowBlur=s(18); ctx.shadowOffsetY=s(5);
        ctx.fillStyle=CLR.white;
        _roundRect(ctx, BX, BY, QR_BOX, QR_BOX, s(12)); ctx.fill();
        ctx.restore();

        // Border gold luar
        const gbq = ctx.createLinearGradient(BX,BY,BX+QR_BOX,BY+QR_BOX);
        gbq.addColorStop(0,CLR.gold3); gbq.addColorStop(0.5,CLR.gold1); gbq.addColorStop(1,CLR.gold3);
        ctx.strokeStyle=gbq; ctx.lineWidth=s(3);
        _roundRect(ctx, BX, BY, QR_BOX, QR_BOX, s(12)); ctx.stroke();
        ctx.strokeStyle='rgba(212,160,23,0.22)'; ctx.lineWidth=s(1.5);
        _roundRect(ctx, BX+s(4), BY+s(4), QR_BOX-s(8), QR_BOX-s(8), s(8)); ctx.stroke();

        // Corner accent
        const CORN = s(16);
        [[0,0],[1,0],[0,1],[1,1]].forEach(([ex,ey]) => {
          const bx2=BX+ex*(QR_BOX-CORN), by2=BY+ey*(QR_BOX-CORN);
          ctx.fillStyle=CLR.gold3; ctx.globalAlpha=0.55;
          ctx.fillRect(bx2, by2, ex===0?CORN:-CORN, s(3));
          ctx.fillRect(bx2, by2, s(3), ey===0?CORN:-CORN);
          ctx.globalAlpha=1;
        });

        // QR image dengan quiet zone ekstra (agar mudah di-scan)
        const QUIET = s(6);
        ctx.drawImage(qrImg, QR_X+QUIET, QR_Y+QR_PAD/2+QUIET, QR_SZ-QUIET*2, QR_SZ-QUIET*2);

        // Label
        const lblY = BY + QR_BOX + s(10);
        ctx.fillStyle=CLR.gray1;
        ctx.textAlign='center'; ctx.textBaseline='top';
        ctx.font=`700 ${s(17)}px Arial, sans-serif`;
        ctx.fillText('Scan untuk verifikasi identitas', cx, lblY);
        ctx.fillStyle=CLR.gray3;
        ctx.font=`500 ${s(13)}px Arial, sans-serif`;
        ctx.fillText('ID: '+(k.id_karyawan||'-'), cx, lblY+s(22));
      }
    } catch(e) { console.warn('[idcard] QR failed:', e.message); }
  }

  // ── FOOTER ────────────────────────────────────────────────
  ctx.fillStyle=CLR.blue2;
  ctx.beginPath();
  ctx.moveTo(0, FOOTER_Y+s(8));
  ctx.quadraticCurveTo(W*0.25, FOOTER_Y-s(16), W*0.5, FOOTER_Y+s(6));
  ctx.quadraticCurveTo(W*0.75, FOOTER_Y+s(22), W, FOOTER_Y-s(6));
  ctx.lineTo(W,H); ctx.lineTo(0,H); ctx.closePath(); ctx.fill();

  const fGrad = ctx.createLinearGradient(0,FOOTER_Y,0,H);
  fGrad.addColorStop(0,CLR.blue2); fGrad.addColorStop(1,CLR.blue1);
  ctx.fillStyle=fGrad;
  ctx.fillRect(0, FOOTER_Y+s(20), W, H-FOOTER_Y-s(20));
  ctx.fillStyle=CLR.gold2;
  ctx.fillRect(0, FOOTER_Y+s(18), W, s(3));
  ...
  if (alamat) {
    ...
    ctx.font=`400 ${s(11)}px Arial, sans-serif`;
    let al=alamat;
    while(ctx.measureText(al).width > W-s(40) && al.length>20)
      al=al.slice(0,-4)+'...';
    ctx.fillText(al, cx, fMidY+s(18));
  }
}
