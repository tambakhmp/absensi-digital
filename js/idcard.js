...
async function renderIDCardToCanvas(canvas, k, instansi, opts) {
  ...
  const HEADER_H  = Math.round(H * 0.30);  // 30% header biru
  const FOOTER_H  = Math.round(H * 0.11);  // 11% footer biru
  ...
  // ── FOTO KARYAWAN (posisi FIXED: center di bawah header) ───
  const FOTO_SZ  = Math.round(W * 0.44);
  const FOTO_CY  = HEADER_H + Math.round(FOTO_SZ * 0.08); // sedikit overlap
  ...
  ctx.restore();
  tY += namaLines.length * nFsz * 1.2 + s(12);
  ...
  ctx.restore();
  ctx.fillStyle = CLR.blue3;
  ctx.fillText(jabatan, cx, tY + s(6));
  tY += jabH + s(8);
  ...
  if (includeQR) {
    try {
      // Hitung ruang tersisa sebelum footer
      const spaceLeft  = FOOTER_Y - s(20) - tY;
      // QR size: ambil 82% dari ruang tersisa, max 460px
      const QR_SZ  = Math.min(s(460), Math.floor(spaceLeft * 0.82));
      const QR_PAD = s(16);
      const QR_BOX = QR_SZ + QR_PAD * 2;

      // Posisi QR: rata tengah di ruang tersisa
      const QR_Y   = tY + Math.floor((spaceLeft - QR_BOX) / 2);
