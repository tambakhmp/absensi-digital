...
async function renderIDCardToCanvas(canvas, k, instansi, opts) {
  ...
  // ══════ FOTO BULAT ══════
  const fotoSize = W * 0.32;
  const fotoX = (W - fotoSize) / 2;
  const fotoY = headerH - fotoSize * 0.35;
  ...
  if (includeQR) {
    try {
      const qrDataUrl = await _generateQRDataURL(k.id_karyawan, s(120));
      if (qrDataUrl) {
        ...
        ctx.lineWidth = s(1);
        ctx.fillRect(qrX - s(6), qrY - s(6), qrSize + s(12), qrSize + s(12));
        ctx.strokeRect(qrX - s(6), qrY - s(6), qrSize + s(12), qrSize + s(12));
