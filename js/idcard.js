...
async function renderIDCardToCanvas(canvas, k, instansi, opts) {
  ...
  // ── HEADER (34% tinggi) ────────────────────────────────────
  const headerH = Math.round(H * 0.34);
  ...
  while (ctx.measureText(namaInst).width > W - s(40) && fSz > s(14)) {
    fSz--;
    ctx.font = `900 ${fSz}px Arial, sans-serif`;
  }
  ...
  // ── FOTO KARYAWAN ──────────────────────────────────────────
  const fotoSz  = Math.round(W * 0.38);  // 38% lebar
  const fotoX   = (W - fotoSz) / 2;
  const fotoY   = headerH - fotoSz * 0.28;  // overlap header
  ...
  while (ctx.measureText(nama).width > W - s(48) && nFsz > s(18)) {
    nFsz--;
    ctx.font = `900 ${nFsz}px Arial, sans-serif`;
  }
  ...
  while (ctx.measureText(jabatan).width > W - s(48) && jFsz > s(11)) {
    jFsz--;
    ctx.font = `700 ${jFsz}px Arial, sans-serif`;
  }
  ...
  ctx.fillText('NIK', boxX + boxW/4, boxY + s(20));
  ctx.fillStyle = CLR.dark;
  ctx.font = `700 ${s(14)}px Arial, sans-serif`;
  ctx.fillText(nik, boxX + boxW/4, boxY + boxH/2 + s(4));

  // Tanggal Masuk (kanan)
  ctx.fillStyle = CLR.gray2;
  ctx.font = `500 ${s(11)}px Arial, sans-serif`;
  ctx.fillText('TGL MASUK', boxX + boxW*3/4, boxY + s(20));
  ctx.fillStyle = CLR.dark;
  ctx.font = `700 ${s(13)}px Arial, sans-serif`;
  ctx.fillText(tglMasuk, boxX + boxW*3/4, boxY + boxH/2 + s(4));
  ...
  if (includeQR) {
    try {
      const qrSz = s(180);
      const qrDataUrl = await _generateQRDataURL(k.id_karyawan, qrSz + s(20));
      if (qrDataUrl) {
        ...
        ctx.font = `400 ${s(10)}px Arial, sans-serif`;
