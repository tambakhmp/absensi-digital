...
async function loadPengajuanSaya() {
  ...
  try {
    // Load pengajuan + surat izin + surat tugas sekaligus
    const [resPgj, resSI, resST] = await Promise.allSettled([
      callAPI('getPengajuanSaya', {}),
    ...
    const siMap = {};
    (resSI.value || []).forEach(s => { if (s.id_pengajuan) siMap[String(s.id_pengajuan)] = s; });

    // Map id_pengajuan → surat tugas
    const stMap = {};
    (resST.value || []).forEach(s => { if (s.id_pengajuan) stMap[String(s.id_pengajuan)] = s; });
    ...
    el.innerHTML = data.slice(0,20).map(p => {
      ...
      const st = p.jenis === 'dinas_luar' ? (stMap[String(p.id_pengajuan)] || null) : null;

      // Surat Izin status badge
      const siStatus = si ? String(si.status_surat || '') : '';
      const siLabel  = si ? (suratLbl[siStatus] || siStatus) : '';
      const siBg     = (siStatus === 'menunggu_admin' || siStatus === 'selesai') ? '#DCFCE7' : '#FEF3C7';
      const siClr    = (siStatus === 'menunggu_admin' || siStatus === 'selesai') ? '#15803D' : '#92400E';

      // Surat Tugas status badge
      const stStatus = st ? String(st.status_surat || '') : '';
      const stLabel  = st ? (suratLbl[stStatus] || stStatus) : '';
      const stBg     = (stStatus === 'menunggu_admin' || stStatus === 'selesai') ? '#DCFCE7' : '#FEF3C7';
      const stClr    = (stStatus === 'menunggu_admin' || stStatus === 'selesai') ? '#15803D' : '#92400E';
      ...
      <div class="pengajuan-item">
        <div class="pengajuan-item__header">
          ...
          ${siLabel ? `<span style="background:${siBg};color:${siClr};font-size:11px;font-weight:600;padding:2px 8px;border-radius:20px;white-space:nowrap">${siLabel}</span>` : ''}
          ${stLabel ? `<span style="background:${stBg};color:${stClr};font-size:11px;font-weight:600;padding:2px 8px;border-radius:20px;white-space:nowrap">${stLabel}</span>` : ''}
        </div>
        ...
        ${(()=>{
          ...
          const idMe = String(sess?.id_karyawan||'');
          const st   = siStatus;
          // Karyawan giliran TTD → tombol mencolok hijau
          if (st==='menunggu_karyawan' && String(si.id_karyawan||'')===idMe)
            return `<button onclick="_lihatSuratIzin('${si.id_surat}')"
              style="margin-top:8px;width:100%;padding:10px;background:#1E5F3A;color:#fff;
              border:none;border-radius:8px;font-size:13px;font-weight:700;cursor:pointer;
              animation:pulse 1.5s ease-in-out infinite">
              ✍️ Tanda Tangani Surat Izin</button>`;
          // Sedang menunggu TTD pihak lain → tombol outline
          if (st==='menunggu_atasan'||st==='menunggu_pimpinan'||st==='menunggu_admin')
            return `<button onclick="_lihatSuratIzin('${si.id_surat}')"
              style="margin-top:8px;width:100%;padding:8px;background:#fff;color:#1E5F3A;
              border:1.5px solid #1E5F3A;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer">
              👁 Lihat Perkembangan Surat Izin</button>`;
          // Selesai → tombol biru (bisa cetak)
          if (st==='selesai')
            return `<button onclick="_lihatSuratIzin('${si.id_surat}')"
              style="margin-top:8px;width:100%;padding:8px;background:#1E3A5F;color:#fff;
              border:none;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer">
              📄 Lihat & Cetak Surat Izin</button>`;
          return `<button onclick="_lihatSuratIzin('${si.id_surat}')"
            style="margin-top:8px;width:100%;padding:8px;background:#F1F5F9;color:#334155;
        ...
        ${(()=>{
          ...
          const idMe = String(sess?.id_karyawan||'');
          const s    = stStatus;
          if (s==='menunggu_karyawan' && String(st.id_karyawan||'')===idMe)
            return `<button onclick="_lihatSuratTugas('${st.id_surat}')"
              style="margin-top:8px;width:100%;padding:10px;background:#1E5F3A;color:#fff;
              border:none;border-radius:8px;font-size:13px;font-weight:700;cursor:pointer;
              animation:pulse 1.5s ease-in-out infinite">
              ✍️ Tanda Tangani Surat Tugas</button>`;
          if (s==='menunggu_atasan'||s==='menunggu_pimpinan'||s==='menunggu_admin')
            return `<button onclick="_lihatSuratTugas('${st.id_surat}')"
              style="margin-top:8px;width:100%;padding:8px;background:#fff;color:#1E5F3A;
              border:1.5px solid #1E5F3A;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer">
              👁 Lihat Perkembangan Surat Tugas</button>`;
          if (s==='selesai')
            return `<button onclick="_lihatSuratTugas('${st.id_surat}')"
              style="margin-top:8px;width:100%;padding:8px;background:#1E3A5F;color:#fff;
              border:none;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer">
              📄 Lihat & Cetak Surat Tugas</button>`;
          return '';
        })()}
        ${p.status === 'pending' && !si && !st ? `<button onclick="batalPengajuanKu('${p.id_pengajuan}')"
        ...
        ${p.status === 'pending' && si && (siStatus === '' || siStatus === 'menunggu_karyawan') ? `
          <button onclick="batalPengajuanKu('${p.id_pengajuan}')"
          style="margin-top:4px;font-size:12px;color:#E53E3E;background:none;border:none;
          cursor:pointer;padding:0">✕ Batalkan pengajuan</button>` : ''}
        ${p.status === 'pending' && st && (stStatus === '' || stStatus === 'menunggu_karyawan') ? `
          <button onclick="batalPengajuanKu('${p.id_pengajuan}')"
          style="margin-top:4px;font-size:12px;color:#E53E3E;background:none;border:none;
          cursor:pointer;padding:0">✕ Batalkan pengajuan</button>` : ''}
      </div>`;
