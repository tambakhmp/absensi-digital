...
async function loadHalamanProfil() {
  ...
  try {
    ...
    elContainer.innerHTML = `
      ...
      <!-- ID CARD KARYAWAN (Portrait + QR) -->
      <div class="card">
        <h3 style="font-size:14px;font-weight:700;color:#64748B;text-transform:uppercase;
          letter-spacing:.6px;margin-bottom:12px;text-align:center">🪪 ID Card Saya</h3>
        <div style="display:flex;justify-content:center;margin-bottom:12px">
          <div id="idcard-profil-preview"
            style="width:100%;max-width:300px;aspect-ratio:400/720;border-radius:16px;
    ...
    // Load ID Card karyawan
    setTimeout(async () => {
      try {
        const instansiData = await callAPI('getMultipleSetting',{
          keys:'nama_instansi,singkatan_instansi,alamat_instansi,logo_url'
        });
        await loadIDCardKaryawan(profil, instansiData);
      } catch(e) { console.warn('load idcard:', e.message); }
    }, 100);
...
async function loadIDCardKaryawan(k, instansi) {
  if (typeof loadIDCardPreview === 'function') {
    loadIDCardPreview('idcard-profil-preview', k, instansi || {}, { showQR: true });
  } else if (typeof _idCardPreviewHTML === 'function') {
    // Fallback kalau idcard.js belum ter-load
    const wrap = document.getElementById('idcard-profil-preview');
    if (wrap) wrap.innerHTML = _idCardPreviewHTML(k, instansi || {});
  }
}
