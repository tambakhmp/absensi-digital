...
async function _prosesAbsenKamera(jenis, posisi) {
  return new Promise(async (resolve, reject) => {
    ...
    modal.innerHTML = `
      ...
      <!-- Peta GPS preview -->
      <div id="gps-map-preview" style="display:none;position:relative;z-index:10"></div>
    ...
    // Tampilkan mini peta posisi GPS di atas tombol kontrol
    const lat = posisi?.coords?.latitude;
    const lon = posisi?.coords?.longitude;
    const acc = posisi?.coords?.accuracy ? Math.round(posisi.coords.accuracy) : null;
    if (lat && lon) {
      const mapWrap = document.getElementById('gps-map-preview');
      if (mapWrap) {
        mapWrap.innerHTML = `
          <div style="position:relative">
            <div id="peta-kamera-absen" style="width:100%;height:130px"></div>
            <div style="position:absolute;top:6px;left:8px;background:rgba(0,0,0,.65);
              ...
              📍 ${lat.toFixed(5)}, ${lon.toFixed(5)}${acc ? ` · ±${acc}m` : ''}
            </div>
            <a href="https://maps.google.com?q=${lat},${lon}" target="_blank"
              style="position:absolute;bottom:6px;right:8px;background:rgba(255,255,255,.9);
              color:#2D6CDF;font-size:10px;padding:3px 8px;border-radius:10px;
              text-decoration:none;font-weight:600;z-index:999">🗺️ Buka Maps</a>
          </div>`;
        mapWrap.style.display = 'block';
        // Init Leaflet map di div (bukan iframe)
        _initLeafletMaps([{ id:'peta-kamera-absen', lat:lat, lon:lon }]);
      }
    }
    ...
    document.getElementById('btn-gunakan-absen').onclick = async () => {
      ...
      try {
        ...
        _tutupKamera();
        // Tampilkan kartu bukti absensi (foto + peta + info)
        _tampilBuktiAbsen(jenis, result, fotoB64, lat, lon);
        resolve(result);
      } catch(e) {
...
// ─── KARTU BUKTI ABSENSI ──────────────────────────────────
function _tampilBuktiAbsen(jenis, result, fotoB64, lat, lon) {
  // Hapus bukti lama kalau ada
  document.getElementById('modal-bukti-absen')?.remove();

  const isWarn   = result.status === 'terlambat';
  const isDinas  = result.status === 'dinas_luar';
  const warna    = isWarn ? '#D97706' : isDinas ? '#EA580C' : '#1A9E74';
  const bg       = isWarn ? '#FFFAF0' : isDinas ? '#FFF3E0' : '#F0FFF4';
  const icon     = isWarn ? '⚠️' : isDinas ? '🚗' : '✅';
  const labelJam = jenis === 'masuk' ? 'Jam Masuk' : 'Jam Keluar';
  const jam      = jenis === 'masuk' ? (result.jam_masuk||'-') : (result.jam_keluar||'-');
  const zona     = result.zona || 'WITA';
  const jarakTxt = (result.jarak && result.jarak > 0) ? `±${result.jarak}m dari kantor` : (isDinas ? 'Dinas Luar' : '');

  // Foto selfie (dari base64 yang diambil)
  const fotoSrc  = fotoB64 ? `data:image/jpeg;base64,${fotoB64}` : '';

  // Peta mini OSM (tanpa API key)
  const petaHtml = (lat && lon)
    ? `<div style="position:relative;border-radius:10px;overflow:hidden;margin-bottom:12px;border:1px solid #E2E8F0">
        <div id="peta-bukti-karyawan" style="width:100%;height:160px"></div>
        <div style="position:absolute;top:6px;left:8px;background:rgba(0,0,0,.65);
          color:#fff;font-size:10px;padding:3px 8px;border-radius:10px;pointer-events:none;z-index:999">
  ...
  // Nama lokasi dari reverse geocode
  const lokasiTxt = result.nama_lokasi
    ? `<div style="font-size:12px;color:#64748B;margin-top:4px">📌 ${result.nama_lokasi}</div>`
    : '';

  const modal = document.createElement('div');
  modal.id    = 'modal-bukti-absen';
  modal.style.cssText = `position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:9800;
    display:flex;align-items:flex-end;backdrop-filter:blur(4px)`;
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:20px 20px 0 0;padding:20px;width:100%;
      ...
      <div style="background:${bg};border-radius:12px;padding:14px 16px;
        ...
        <div>
          <div style="font-size:15px;font-weight:700;color:${warna}">
            Absen ${jenis === 'masuk' ? 'Masuk' : 'Keluar'} Berhasil</div>
          <div style="font-size:13px;color:${warna};margin-top:2px">
            ${labelJam}: <strong>${jam} ${zona}</strong>
            ${jarakTxt ? ` · ${jarakTxt}` : ''}</div>
        </div>
      </div>

      <!-- Foto selfie + peta berdampingan (tablet) atau stacked (mobile) -->
      <div style="display:grid;grid-template-columns:${fotoSrc?'1fr 1fr':'1fr'};gap:10px;margin-bottom:12px">
        ${fotoSrc ? `
        <div>
          <div style="font-size:11px;font-weight:600;color:#64748B;margin-bottom:6px">📸 Foto Selfie</div>
          <img src="${fotoSrc}"
            style="width:100%;border-radius:10px;object-fit:cover;max-height:200px;
            border:2px solid #E2E8F0">
        </div>` : ''}
        <div>
          <div style="font-size:11px;font-weight:600;color:#64748B;margin-bottom:6px">📍 Posisi GPS</div>
          ${petaHtml || `<div style="background:#F8FAFC;border-radius:10px;padding:20px;
            text-align:center;color:#94A3B8;font-size:12px">GPS tidak tersedia</div>`}
          ${lokasiTxt}
        </div>
      </div>

      <button class="btn btn--primary btn--full btn--lg"
        onclick="document.getElementById('modal-bukti-absen').remove();loadStatusAbsenHariIni()">
        ✓ Tutup
      </button>
    </div>`;
  ...
  // Init Leaflet map setelah DOM siap
  if (lat && lon) {
    if (typeof _initLeafletMaps === 'function') {
      _initLeafletMaps([{ id:'peta-bukti-karyawan', lat:lat, lon:lon }]);
    }
  }

  // Auto tutup setelah 30 detik
  setTimeout(function() {
    const m = document.getElementById('modal-bukti-absen');
    if (m) m.remove();
    loadStatusAbsenHariIni();
  }, 30000);
}

// ─── LEAFLET MAP HELPER (shared, cek dulu sebelum load ulang) ───
function _initLeafletMaps(maps) {
  ...
  if (window.L) {
    ...
    s.onload   = ()=>setTimeout(doInit,150);
    s.onerror  = ()=>console.warn('Leaflet gagal dimuat');
    document.head.appendChild(s);
  } else {
    // Script sedang loading, tunggu
    setTimeout(()=>{ if(window.L) doInit(); }, 600);
  }
}
