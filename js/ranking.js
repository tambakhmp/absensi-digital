  ...
  try {
    ...
    if (!data || data.length === 0) {
      el.innerHTML = `<div class="card" style="text-align:center;padding:20px;color:#94A3B8">
        ...
        <div style="font-size:13px">Belum ada data absensi bulan ini untuk dihitung ranking.</div>
      </div>`;
      return;
    }
      ...
      <div class="card">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">
          ...
          <div style="text-align:right">
            <span style="font-size:11px;background:#EBF8EE;color:#1A9E74;padding:2px 8px;border-radius:10px;font-weight:600">⚡ Real-time</span>
            <div style="font-size:10px;color:#94A3B8;margin-top:2px">${n} karyawan</div>
          </div>
        </div>
...
function _rankCard(r) {
  ...
  return `<div style="display:flex;align-items:center;gap:12px;padding:10px 0;
    ...
    <div style="text-align:right;flex-shrink:0">
      <div style="font-size:16px;font-weight:700;color:var(--color-primer)">${parseInt(r.skor_total||r.skor_kehadiran||0)}<span style="font-size:10px;color:#94A3B8">/100</span></div>
      <div style="font-size:10px;color:#1A9E74">✅ ${r.total_hadir||0} ⏰ ${r.total_terlambat||0}</div>
      <div style="font-size:10px;color:#E53E3E">❌ ${r.total_alfa||0} alfa</div>
    </div>
  </div>`;
}
...
function _rankCardBuruk(r) {
  ...
  return `<div style="display:flex;align-items:center;gap:12px;padding:10px 0;
    ...
    <div style="text-align:right;flex-shrink:0">
      <div style="font-size:16px;font-weight:700;color:#E53E3E">${parseInt(r.skor_total||r.skor_kehadiran||0)}<span style="font-size:10px;color:#94A3B8">/100</span></div>
