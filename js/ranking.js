...
async function renderRankingSection(containerId) {
  ...
  try {
    ...
    if (!data || data.length === 0) {
      ...
      if (el2) el2.innerHTML = `<div class="card" style="text-align:center;padding:20px;color:#94A3B8">
        <div style="font-size:28px;margin-bottom:8px">🏆</div>
        <div style="font-size:13px">Ranking belum dihitung bulan ini.<br>
        Admin dapat menghitung di menu <strong>Ranking → Hitung Ulang</strong>.</div>
      </div>`;
      return;
    }
    const now   = new Date();
