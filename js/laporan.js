...
async function cetakRekapPDF(idKaryawan, bulan, tahun, tanggalDari, tanggalKe) {
  ...
  try {
    ...
    // Format: "25 Apr 2026 s/d 24 Mei 2026"
    const _fmtPeriode = (tgl) => {
      const p = String(tgl||'').split('/');
      if (p.length < 3) return tgl;
      return parseInt(p[0]) + ' ' + BULAN_PENDEK[parseInt(p[1])-1] + ' ' + p[2];
    };

    const periodeLabel = (tanggalDari && tanggalKe)
      ? _fmtPeriode(data.tgl_dari || tanggalDari) + ' s/d ' + _fmtPeriode(data.tgl_ke || tanggalKe)
      : (bulan ? BULAN_PENDEK[parseInt(bulan)-1] + ' ' + tahun : '');
    ...
    doc.setFontSize(10);
    // ── DATA KARYAWAN ──
    // Label rata KIRI, titik dua di posisi tetap (padding spasi), nilai rata kiri
    // Kolom kiri : label x=mL,    titik dua x=mL+28,   nilai x=mL+32
    // Kolom kanan: label x=W/2+5, titik dua x=W/2+33,  nilai x=W/2+37
    ...
    const rowKaryawan = [
      ...
      [['No. HP',       String(k.no_hp||'-')],      ['Email',       String(k.email||'-')]]
    ];

    doc.setFontSize(9.5);
    rowKaryawan.forEach(function(row) {
      // Kolom kiri: label rata kiri, ':' posisi tetap, nilai rata kiri
      doc.setFont('helvetica','bold');
      doc.text(row[0][0], xLbl1, y);
      doc.text(':', xColon1, y);
      doc.setFont('helvetica','normal');
      var lines1 = doc.splitTextToSize(String(row[0][1]||'-'), xLbl2 - xVal1 - 4);
      lines1.slice(0,2).forEach(function(l,i){ doc.text(l, xVal1, y + i*4.5); });

      // Kolom kanan: label rata kiri, ':' posisi tetap, nilai rata kiri
      doc.setFont('helvetica','bold');
      doc.text(row[1][0], xLbl2, y);
      doc.text(':', xColon2, y);
      doc.setFont('helvetica','normal');
      var lines2 = doc.splitTextToSize(String(row[1][1]||'-'), W - mR - xVal2);
      lines2.slice(0,2).forEach(function(l,i){ doc.text(l, xVal2, y + i*4.5); });
    ...
    // Ringkasan KOMPAK: 7 item 1 baris, hemat ruang
    const ringN    = sts.length;
    const ringColW = (W - mL - mR) / ringN;
    const ringH    = 13; // cukup untuk label + angka
    sts.forEach(function(s, i) {
      const cx  = mL + i * ringColW;
      const hex = s[2];
      ...
      doc.rect(cx, y-3, ringColW-1, ringH, 'F');
      doc.setTextColor(255,255,255);
      doc.setFont('helvetica','bold');
      doc.setFontSize(7);
      doc.text(s[0], cx + (ringColW-1)/2, y+1.5, {align:'center'});
      doc.setFontSize(11);
      doc.text(String(s[1]), cx + (ringColW-1)/2, y+8, {align:'center'});
    });
    ...
    y += 4;  // jarak dari garis sebelumnya
    doc.setFont('helvetica','bold');
    doc.setFontSize(10);
    doc.text('RINCIAN HARIAN', mL, y);
    y += 6;

    // Header tabel — kolom kecil dipersempit, keterangan diperlebar
    const cols = [
      { label:'No',       w:5,  x:mL },
      { label:'Tanggal',  w:20, x:mL+5 },
      { label:'Hari',     w:12, x:mL+25 },
      { label:'Masuk',    w:13, x:mL+37 },
      { label:'Keluar',   w:13, x:mL+50 },
      { label:'Status',   w:20, x:mL+63 },
      { label:'Keterangan & Lokasi GPS', w:W-mR-mL-63-20, x:mL+83 }
    ];
    // Keterangan width = 170-83 = 87mm (lebih lebar dari sebelumnya 73mm)
    ...
    // Fix 6: Multi page - tambah header tabel di halaman baru
    ...
    const _addTableHeader = () => {
      ...
      doc.setFontSize(9);
      doc.setFont('helvetica','bold');
      cols.forEach(function(c) { doc.text(c.label, c.x+1, y); });
      doc.setTextColor(0,0,0);
      y += 4;
    };
    ...
    data.absensi.forEach(function(a, i) {
      if (y > PAGE_BREAK_Y) {
        doc.addPage();
        y = 20;
        // Header kolom di halaman baru
        doc.setFont('helvetica','bold');
        doc.setFontSize(9);
        doc.setTextColor(100,100,100);
        doc.text('(Lanjutan) RINCIAN HARIAN', mL, y);
        y += 5;
        _addTableHeader();
      }
      // Keterangan: tampilkan hanya lokasi, hapus alasan dan info tambahan
      var _ketFull = String(a.keterangan||'').replace(/AUTO_KELUAR:[\d:]+\s*\|?\s*/,'').trim();
      var ketRaw;
      if (_ketFull.toLowerCase().indexOf('dinas luar') === 0) {
        // Format: "Dinas Luar @ Lokasi (WIB) s/d tgl: alasan..."
        // Ambil hanya sampai sebelum "(WIB)", "s/d", atau ":"
        var _loc = _ketFull;
        _loc = _loc.replace(/\s*\([^)]*\).*$/,'');  // hapus (WIB) dan sesudahnya
        _loc = _loc.replace(/\s*s\/d\s*\d.*/,'');   // hapus "s/d tanggal" dan sesudahnya
        _loc = _loc.replace(/\s*:\s*.*/,'');           // hapus ": alasan"
        ketRaw = _loc.trim() || _ketFull;
      } else {
        ketRaw = _ketFull || '-';
      }
      var lat     = String(a.lat_masuk  || '');
      var lon     = String(a.lon_masuk  || '');
      var hasGPS  = lat && lon && lat !== '' && lon !== '';
      var gpsStr  = hasGPS ? ('GPS: ' + parseFloat(lat).toFixed(4) + ', ' + parseFloat(lon).toFixed(4)) : '';
      var linkStr = hasGPS ? ('maps.google.com/?q=' + parseFloat(lat).toFixed(6) + ',' + parseFloat(lon).toFixed(6)) : '';

      // Wrap keterangan agar tidak overflow
      var ketW    = cols[6].w - 1;  // 86mm — manfaatkan ruang penuh
      doc.setFont('helvetica','normal');
      doc.setFontSize(8);
      var ketLines = doc.splitTextToSize(ketRaw, ketW);
      if (ketLines.length > 2) ketLines = ketLines.slice(0,2);

      // Tinggi baris: 1 baris = 6.5, ada GPS = +3.5 per baris GPS
      var rowH = 6.5;
      if (ketLines.length > 1) rowH += 3.5;
      if (hasGPS) rowH += 3.5 + (linkStr ? 3.5 : 0);
      ...
      doc.setFont('helvetica','normal');
      doc.setFontSize(8.5);
      // Kolom fix — center vertical di rowH
      var midY = y + (rowH - 6.5) / 2;
      doc.text(String(i+1),                                         cols[0].x+1, midY);
      doc.text(String(a.tanggal||'-'),                              cols[1].x+1, midY);
      doc.text(String(a.hari||'-'),                                 cols[2].x+1, midY);
      doc.text(String(a.jam_masuk||'-'),                            cols[3].x+1, midY);
      doc.text(String(a.jam_keluar||'-'),                           cols[4].x+1, midY);
      doc.setFontSize(8);
      doc.text(statusLabel[a.status]||String(a.status||'-'),        cols[5].x+1, midY);
      ...
      var ketY = y;
      ketLines.forEach(function(ln) {
        doc.text(ln, cols[6].x+1, ketY);
        ketY += 3.5;
      });

      // GPS koordinat (lebih kecil, warna abu)
      if (hasGPS) {
        doc.setFontSize(7);
        doc.setTextColor(80,80,80);
        doc.text(gpsStr, cols[6].x+1, ketY);
        ketY += 3.5;
        if (linkStr) {
          doc.setTextColor(45,108,223);
          doc.text(linkStr, cols[6].x+1, ketY);
        }
      }
      doc.setTextColor(0,0,0);
      y += rowH;
    ...
    doc.setFontSize(8.5);
    doc.setTextColor(45,108,223);
    const totalHadir = (data.ringkasan.hadir||0) + (data.ringkasan.terlambat||0) + (data.ringkasan.dinas_luar||0);
    const totalHari  = data.absensi.length;
    const pct = totalHari > 0 ? Math.round(totalHadir/totalHari*100) : 0;
    doc.text('TOTAL: ' + totalHari + ' hari', cols[1].x+1, y+1);
    doc.text('Hadir: ' + totalHadir + ' (' + pct + '%)', cols[3].x+1, y+1);
    doc.text('Alfa: ' + (data.ringkasan.alfa||0), cols[5].x+1, y+1);
    doc.setTextColor(0,0,0);
    y += 6;

    // Border tabel dihapus - tidak konsisten di multi-page
    // Garis bawah tabel saja
    doc.setLineWidth(0.3);
    ...
    // Fix 4: Nama atasan dari data.atasan
    // Fix 5: HRD dan Pimpinan dikosongkan (bukan superadmin)
    const namaAtasan   = data.atasan?.nama_lengkap || k.nama_atasan || '(.....................)';
    const namaHRD      = data.hrd?.nama_lengkap || '';       // kosong kalau superadmin
    const namaPimpinan = data.pimpinan?.nama_lengkap || '';   // kosong kalau superadmin

    // Filter: jangan tampilkan "Super Administrator" di TTD
    const _filterNama = (nama) => {
      if (!nama) return '';
      if (/super.?admin/i.test(nama)) return '';
      return nama;
    };

    y = await _kolomTTD(doc, [
      { label: 'Karyawan', nama: k.nama_lengkap,           ttd: k.tanda_tangan_url },
      { label: 'Atasan',   nama: _filterNama(namaAtasan),  ttd: data.atasan?.tanda_tangan_url },
      { label: 'HRD',      nama: _filterNama(namaHRD),     ttd: data.hrd?.tanda_tangan_url },
      { label: 'Pimpinan', nama: _filterNama(namaPimpinan),ttd: data.pimpinan?.tanda_tangan_url }
    ], mL, W-mR, y, doc, false);
...
async function exportRekapExcel(bulan, tahun, tanggalDari, tanggalKe) {
  ...
  try {
    ...
    // Helper: ambil kode status — bila status kosong tapi jam_masuk ada → 'H'
    const getKodeStatus = (a) => {
      const st = String(a.status || '').toLowerCase().trim();
      if (st && STATUS_KODE[st]) return STATUS_KODE[st];
      // Status kosong/tidak dikenal tapi ada jam_masuk → hadir
      if (!st && String(a.jam_masuk||'').trim()) return 'H';
      return STATUS_KODE[st] || '';
    };
...
async function cetakRekapLemburPDF(bulan, tahun, tanggalDari, tanggalKe) {
  ...
  try {
    ...
    const BULAN_L = ['Januari','Februari','Maret','April','Mei','Juni',
                    'Juli','Agustus','September','Oktober','November','Desember'];
    const BULAN_PENDEK_L = ['Jan','Feb','Mar','Apr','Mei','Jun',
                            'Jul','Agu','Sep','Okt','Nov','Des'];
    ...
    const _fmtPL = (tgl) => {
      const p = String(tgl||'').split('/');
      if (p.length < 3) return tgl;
      return parseInt(p[0]) + ' ' + BULAN_PENDEK_L[parseInt(p[1])-1] + ' ' + p[2];
    };

    const label = (tanggalDari && tanggalKe)
      ? _fmtPL(data.tgl_dari || tanggalDari) + ' s/d ' + _fmtPL(data.tgl_ke || tanggalKe)
      : (BULAN_L[parseInt(bulan)-1] || '') + ' ' + (tahun || '');
