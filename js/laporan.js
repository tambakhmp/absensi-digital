...
async function cetakRekapPDF(idKaryawan, bulan, tahun, tanggalDari, tanggalKe) {
  ...
  try {
    ...
    sts.forEach(function(s, i) {
      ...
      doc.setFont('helvetica','bold');
      doc.setFontSize(8);
      // Label di atas - proporsional tengah
      doc.setFontSize(8);
      doc.text(s[0], cx + (ringColW-2)/2, cy + 4, {align:'center'});
      // Angka besar di tengah
      doc.setFontSize(18);
      doc.setFont('helvetica','bold');
      doc.text(String(s[1]), cx + (ringColW-2)/2, cy + 11.5, {align:'center'});
    });
