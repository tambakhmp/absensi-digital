...
async function cetakRekapPDF(idKaryawan, bulan, tahun, tanggalDari, tanggalKe) {
  ...
  try {
    ...
    doc.setFontSize(10);
    // ── DATA KARYAWAN — tabel 3 kolom: label | : | nilai ──
    // Kolom kiri : label rata kanan di x=50, ':' di x=52, nilai rata kiri di x=56
    // Kolom kanan: label rata kanan di x=140, ':' di x=142, nilai rata kiri di x=146
    const COL = [
      { xLabel: mL+30, xColon: mL+31, xVal: mL+34 },          // kolom kiri
      { xLabel: W/2+30, xColon: W/2+31, xVal: W/2+34 }        // kolom kanan
    ];
    ...
    rowKaryawan.forEach(function(row) {
      [0,1].forEach(function(ci) {
        var c    = COL[ci];
        var lbl  = row[ci][0];
        var val  = String(row[ci][1]||'-');

        // Label: rata kanan menuju titik dua
        doc.setFont('helvetica','bold');
        doc.text(lbl, c.xColon - 2, y, {align:'right'});

        // Titik dua: posisi tetap
        doc.text(':', c.xColon, y);

        // Nilai: rata kiri dari posisi tetap
        doc.setFont('helvetica','normal');
        // Batasi lebar nilai agar tidak melewati batas kolom
        var maxW = ci === 0 ? (W/2 - mL - 34) : (W - mR - W/2 - 34);
        var lines = doc.splitTextToSize(val, maxW);
        lines.slice(0,2).forEach(function(line, li) {
          doc.text(line, c.xVal, y + li * 4.5);
        });
      });
      y += 7;
    });
