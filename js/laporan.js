...
async function cetakRekapPDF(idKaryawan, bulan, tahun, tanggalDari, tanggalKe) {
  ...
  try {
    ...
    doc.setFontSize(10);
    // Layout sejajar: label right-aligned ke titik dua, nilai left dari titik dua
    // Kolom kiri : label  x=20-51, titik dua x=52, nilai x=55
    // Kolom kanan: label x=110-136, titik dua x=137, nilai x=140
    const xColon1 = mL + 32;    // posisi ':' kolom kiri
    const xVal1   = mL + 35;    // posisi nilai kolom kiri
    const xColon2 = W/2 + 32;   // posisi ':' kolom kanan
    const xVal2   = W/2 + 35;   // posisi nilai kolom kanan
    const maxVal1 = 36;          // max karakter nilai kolom kiri
    const maxVal2 = 36;          // max karakter nilai kolom kanan
    ...
    rowKaryawan.forEach(function(row) {
      // ── Kolom kiri ──
      doc.setFont('helvetica','bold');
      // Label right-aligned ke titik dua
      doc.text(row[0][0], xColon1, y, {align:'right'});
      doc.text(':', xColon1, y);
      doc.setFont('helvetica','normal');
      doc.text(String(row[0][1]||'-').substring(0, maxVal1), xVal1, y);

      // ── Kolom kanan ──
      doc.setFont('helvetica','bold');
      doc.text(row[1][0], xColon2, y, {align:'right'});
      doc.text(':', xColon2, y);
      doc.setFont('helvetica','normal');
      // Email: tampilkan penuh, wrap ke baris bawah kalau perlu
      const val2 = String(row[1][1]||'-');
      if (val2.length > maxVal2) {
        doc.text(val2.substring(0, maxVal2), xVal2, y);
        doc.text(val2.substring(maxVal2, maxVal2*2), xVal2, y+4);
      } else {
        doc.text(val2, xVal2, y);
      }
