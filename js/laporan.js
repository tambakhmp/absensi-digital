...
async function cetakRekapPDF(idKaryawan, bulan, tahun, tanggalDari, tanggalKe) {
  ...
  try {
    ...
    doc.setFontSize(10);
    // Posisi tetap agar sejajar sempurna
    // Kolom kiri: label x=20, nilai x=52
    // Kolom kanan: label x=110, nilai x=142
    const xL1 = mL;        // label kiri
    const xV1 = mL + 32;   // nilai kiri
    const xL2 = W/2 + 5;   // label kanan
    const xV2 = W/2 + 37;  // nilai kanan
    ...
    rowKaryawan.forEach(function(row) {
      ...
      doc.setFont('helvetica','bold');
      doc.setFontSize(9.5);
      doc.text(row[0][0] + ':', xL1, y);
      doc.setFont('helvetica','normal');
      // Potong nilai kalau terlalu panjang
      const val1 = String(row[0][1]||'-');
      doc.text(val1.substring(0, 32), xV1, y);

      // Kolom kanan
      doc.setFont('helvetica','bold');
      doc.text(row[1][0] + ':', xL2, y);
      doc.setFont('helvetica','normal');
      const val2 = String(row[1][1]||'-');
      // Email bisa panjang - potong 28 char
      const maxV2 = row[1][0] === 'Email' ? 28 : 35;
      doc.text(val2.substring(0, maxV2), xV2, y);
