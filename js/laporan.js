...
async function cetakRekapPDF(idKaryawan, bulan, tahun, tanggalDari, tanggalKe) {
  ...
  try {
    ...
    const ringH    = 11; // lebih kecil dari 15
    sts.forEach(function(s, i) {
      ...
      doc.setFontSize(13);
      doc.text(String(s[1]), cx + (ringColW-1)/2, y+8.5, {align:'center'});
    });
    ...
    data.absensi.forEach(function(a, i) {
      ...
      // Hitung keterangan full (hapus prefix AUTO_KELUAR jika ada)
      var ketRaw  = String(a.keterangan||'').replace(/AUTO_KELUAR:[\d:]+\s*\|?\s*/,'').trim() || '-';
