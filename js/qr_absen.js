...
async function _ambilFotoKaryawan(jenis) {
  ...
  // Compress foto: resize max 640px supaya payload tidak terlalu besar
  const MAX_SIZE = 640;
  ...
  // JPEG quality 0.6 — balance antara kualitas dan ukuran file
  ...
  if (btnAmbil) {
    btnAmbil.innerHTML = '<div class="spinner-btn" style="display:inline-block;width:18px;height:18px;border:2px solid rgba(15,23,42,.2);border-top-color:#0F172A;border-radius:50%;animation:spin .8s linear infinite"></div> <span>Mengirim...</span>';
  ...
  try {
    ...
    if (!result) {
      // callAPI return null = session expired / logout triggered
      // Tidak perlu do apa-apa, window sudah reload
      return;
    }

    _tampilkanSukses(result, jenis);
  } catch(e) {
    console.error('[QR submit]', e);
