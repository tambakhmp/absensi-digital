...
async function _pilihJenisAbsen(jenis) {
  ...
  container.innerHTML = `
    <div style="height:100%;display:flex;flex-direction:column;background:#000">
      ...
      <div style="background:#0F172A;padding:16px;display:flex;gap:10px;align-items:center;flex-shrink:0">
        ...
        <button onclick="_ambilFotoKaryawan('${jenis}')" id="btn-ambil-foto"
  ...
  try {
    ...
    if (video) video.srcObject = _qrMediaStream;
  } catch(e) {
    showToast('Gagal buka kamera: ' + e.message, 'error', 5000);
    _tampilkanKonfirmasi();
  }
...
async function _ambilFotoKaryawan(jenis) {
  ...
  // Capture ke canvas — resize max 800px untuk kecilkan payload
  const videoW = video.videoWidth || 720;
  const videoH = video.videoHeight || 1280;
  const MAX_WIDTH = 800;
  const ratio = Math.min(MAX_WIDTH / videoW, 1);
  canvas.width = Math.round(videoW * ratio);
  canvas.height = Math.round(videoH * ratio);
  const ctx = canvas.getContext('2d');
  ctx.drawImage(video, 0, 0, canvas.width, canvas.height);

  // Compress jadi JPEG 60% — target <100KB
  const fotoBase64 = canvas.toDataURL('image/jpeg', 0.6);
  const sizeKB = Math.round(fotoBase64.length * 0.75 / 1024);
  console.log('[QR foto] Ukuran:', sizeKB, 'KB (', canvas.width, 'x', canvas.height, ')');
  ...
  try {
    ...
    _tampilkanError(e.message, jenis);
  }
}
