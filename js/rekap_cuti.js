...
async function exportCutiExcel() {
  ...
  _cutiData.forEach((k, i) => {
    ...
    const periodeX = detailX.map(function(d) {
      return d.tanggal_mulai + (d.tanggal_selesai && d.tanggal_selesai!==d.tanggal_mulai
        ? ' s/d '+d.tanggal_selesai : '') + (d.total_hari?' ('+d.total_hari+' hr)':'');
    }).join(', ') || '-';
    ...
    _cutiData.forEach((k, i) => {
      ...
      if (detail.length > 0) {
        ...
        detail.forEach(function(d) {
          if (d.tanggal_mulai) {
            const line = d.tanggal_mulai + (d.tanggal_selesai && d.tanggal_selesai !== d.tanggal_mulai
              ? ' s/d ' + d.tanggal_selesai : '') +
