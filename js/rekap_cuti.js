...
async function exportCutiExcel() {
  ...
  const _nowX = new Date();
  const BLN_X = ['Januari','Februari','Maret','April','Mei','Juni',
    'Juli','Agustus','September','Oktober','November','Desember'];
  const tglCetak = _nowX.getDate()+' '+BLN_X[_nowX.getMonth()]+' '+_nowX.getFullYear();
  ...
  const aoa = [
    ...
    [],                                        // row 8: kosong
    // row 9: header tabel
    ['No','NIK','Nama Karyawan','Jabatan','Departemen','Jatah (hari)','Terpakai (hari)','Sisa (hari)','Periode Cuti','Status','Pending (hari)'],
  ];
  ...
  _cutiData.forEach((k, i) => {
    ...
    // Format periode cuti
    const detailX = k.detail || [];
    const periodeX = detailX.map(function(d) {
      const tM = _fmtTanggal(d.tanggal_mulai);
      const tK = _fmtTanggal(d.tanggal_selesai);
      return tM + (tK && tK!==tM ? ' s/d '+tK : '') + (d.total_hari?' ('+d.total_hari+' hr)':'');
    }).join(' | ') || '-';

    aoa.push([
      i+1, k.nik||'-', k.nama_lengkap||'-',
      k.jabatan||'-', k.departemen||'-',
      jatah, terpakai, sisa, periodeX, status, pending
    ]);
  });
  ...
  ws['!cols'] = [
    ...
    {wch:8},{wch:10},{wch:8},{wch:32},{wch:10},{wch:9}
  ];
  // Tinggi baris
  ws['!rows'] = [
...
// Helper format tanggal: "01/05/2026" atau "2026-05-01" → "1 Mei 2026"
function _fmtTanggal(t) {
  if (!t || t === '-') return '-';
...
async function exportCutiPDF() {
  ...
  try {
    ...
    const _now = new Date();
    const BLN_PDF = ['Januari','Februari','Maret','April','Mei','Juni',
      'Juli','Agustus','September','Oktober','November','Desember'];
    const tglCetakPDF = _now.getDate()+' '+BLN_PDF[_now.getMonth()]+' '+_now.getFullYear();
    doc.text('Dicetak: ' + tglCetakPDF, W/2, y, {align:'center'});
    ...
    // ── Kolom tabel ─────────────────────────────────────────
    // Landscape A4 (297mm) untuk muat semua kolom
    const cols = [
      {x:mL,     w:6,  label:'No',             align:'center'},
      {x:mL+7,   w:24, label:'NIK',            align:'left'},
      {x:mL+32,  w:35, label:'Nama Karyawan',  align:'left'},
      {x:mL+68,  w:28, label:'Jabatan',        align:'left'},
      {x:mL+97,  w:22, label:'Departemen',     align:'left'},
      {x:mL+120, w:11, label:'Jatah',          align:'center'},
      {x:mL+132, w:12, label:'Terpakai',       align:'center'},
      {x:mL+145, w:10, label:'Sisa',           align:'center'},
      {x:mL+156, w:57, label:'Periode Cuti (Tanggal)',  align:'left'},
      {x:mL+214, w:22, label:'Status',         align:'center'},
    ];
    const ROW_H = 6.5;
    ...
    _cutiData.forEach((k, i) => {
      ...
      // NIK — paksa 1 baris (font kecil sampai muat)
      let nikTxt = String(k.nik||'-');
      let nikFs  = fs;
      doc.setFontSize(nikFs);
      while (doc.getTextWidth(nikTxt) > cols[1].w-2 && nikFs > 6) {
        nikFs -= 0.5; doc.setFontSize(nikFs);
      }
      doc.text(nikTxt, cols[1].x+1, midY);
      doc.setFontSize(fs);
      ...
      // Periode cuti — tampilkan semua tanggal cuti yang disetujui
      ...
      if (detail.length > 0) {
        ...
        detail.forEach(function(d) {
          if (d.tanggal_mulai) {
            const tM = _fmtTanggal(d.tanggal_mulai);
            const tK = _fmtTanggal(d.tanggal_selesai);
            const line = tM + (tK && tK !== tM ? ' s/d ' + tK : '') +
              (d.total_hari ? ' (' + d.total_hari + ' hr)' : '');
            periodeLines.push(line);
          }
        });
        doc.setFontSize(7.5);
        const startY2 = y - 3 + (rowH - periodeLines.length * 3.2) / 2 + 3;
        periodeLines.forEach(function(ln, li) {
          doc.text(ln, cols[8].x+1, startY2 + li*3.2);
        });
        doc.setFontSize(fs);
      } else {
        doc.setFontSize(7.5);
        doc.setTextColor(180,180,180);
        doc.text('-', cols[8].x+cols[8].w/2, midY, {align:'center'});
        doc.setTextColor(30,30,30); doc.setFontSize(fs);
      }

      // Badge status — kolom ke-9 (index 9)
      const badgeH  = Math.min(5.5, rowH-2);
      const badgeY  = y - 3 + (rowH - badgeH)/2;
      const badgeW  = cols[9].w - 3;
      doc.setFillColor(...sColor);
      doc.roundedRect(cols[9].x+1.5, badgeY, badgeW, badgeH, 1.5, 1.5, 'F');
      doc.setTextColor(255,255,255);
      doc.setFont('helvetica','bold');
      const bFsz = Math.min(7.5, badgeH*0.55+3.5);
      doc.setFontSize(bFsz);
      doc.text(status, cols[9].x+cols[9].w/2, badgeY+badgeH/2+bFsz*0.18, {align:'center'});
