  ...
  container.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:gap:8px">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <button class="btn btn--ghost" style="font-size:13px;background:#ECFDF5;color:#065F46;border:1px solid #10B981"
          onclick="bukaModalImportCSV()">
          📤 Import CSV
        </button>
...
function bukaModalImportCSV() {
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:16px;padding:24px;width:100%;max-width:560px;
      max-height:92vh;overflow-y:auto;animation:fadeInScale .2s ease">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
        <h3 style="font-size:17px;font-weight:700;margin:0">📤 Import Karyawan via CSV</h3>
        <button onclick="document.getElementById('modal-import-csv').remove()"
      ...
      <div id="csv-step-1">
        <div style="background:#ECFDF5;border:1px solid #10B981;border-radius:10px;padding:14px;
          font-size:13px;color:#065F46;margin-bottom:16px;line-height:1.7">
          <strong>📋 Cara pakai:</strong>
          <ol style="margin:6px 0 0 20px;padding:0;line-height:1.6">
            <li>Download template CSV di bawah</li>
            <li>Isi data karyawan di template</li>
            <li>Simpan sebagai .csv lalu upload di sini</li>
          </ol>
          <div style="margin-top:10px;padding:8px;background:#fff;border-radius:6px;font-size:12px">
            <strong>⚙️ Default otomatis:</strong><br>
            • Username = dari nama (contoh: "Budi S." → <code>budi.s</code>)<br>
            • Password semua: <code style="color:#065F46;font-weight:700">Absensi@123</code><br>
            • Role default: <code>karyawan</code>
          </div>
        </div>

        <button class="btn btn--ghost btn--full" onclick="downloadTemplateCSV()" style="margin-bottom:12px">
        ...
        <div class="form-group">
          ...
          <input type="file" id="csv-file-input" accept=".csv,text/csv"
            onchange="previewCSV(this)" class="form-control">
        </div>
...
function downloadTemplateCSV() {
  const headers = [
    'nama_lengkap','nik','email','no_hp','jabatan','departemen',
    'tanggal_masuk','tanggal_lahir','jenis_kelamin','alamat',
    'pendidikan_terakhir','no_ktp','nama_atasan','role','id_shift'
  ];
  const contoh = [
    [
      'Budi Santoso','','budi@instansi.id','081234567890','Staff','Operations',
      '01/03/2024','15/05/1995','L','Jl. Merdeka No. 1',
      'S1','3201234567890001','Nama Atasan','karyawan',''
    ],
    [
      'Ani Putri','','ani@instansi.id','081298765432','Staff','Finance',
      '01/03/2024','20/08/1996','P','Jl. Sudirman No. 5',
      'S1','','','karyawan',''
    ]
  ];
  ...
  // BOM agar Excel parse UTF-8 benar
...
function previewCSV(input) {
  ...
  reader.onload = (e) => {
    try {
      ...
      if (rows.length < 2) {
        showToast('File CSV kosong atau hanya header', 'warning');
        return;
      }
      ...
      for (let i = 1; i < rows.length; i++) {
        const row = rows[i];
        if (row.every(c => !c || !String(c).trim())) continue; // skip baris kosong
        const obj = {};
        headers.forEach((h, idx) => { obj[h] = row[idx] || ''; });
        _csvParsedRows.push(obj);
      }
      _renderCSVPreview();
    } catch(err) {
...
// Simple CSV parser (support quoted fields with commas)
...
function _renderCSVPreview() {
  ...
  if (!el || _csvParsedRows.length === 0) return;

  const previewRows = _csvParsedRows.slice(0, 5);
  el.style.display = 'block';
  el.innerHTML = `
    <div style="background:#F8FAFC;border-radius:10px;padding:14px;margin:12px 0">
      <div style="font-weight:700;font-size:13px;margin-bottom:8px;color:#1E293B">
        📋 Preview (${_csvParsedRows.length} baris akan diimport, menampilkan 5 pertama):
      </div>
        ...
        <table style="width:100%;font-size:12px;border-collapse:collapse">
          <thead>
            <tr style="background:#E2E8F0">
              ...
              <th style="padding:6px 8px;text-align:left">NIK</th>
              <th style="padding:6px 8px;text-align:left">Jabatan</th>
              <th style="padding:6px 8px;text-align:left">Departemen</th>
            </tr>
          </thead>
          <tbody>
            ${previewRows.map((r, i) => `
              <tr style="border-bottom:1px solid #E2E8F0">
                ...
                <td style="padding:6px 8px">${r.nama_lengkap || '<em style=color:#EF4444>kosong</em>'}</td>
                <td style="padding:6px 8px">${r.nik || '<em style=color:#94A3B8>auto</em>'}</td>
                <td style="padding:6px 8px">${r.jabatan || '<em style=color:#94A3B8>Staff</em>'}</td>
                <td style="padding:6px 8px">${r.departemen || '<em style=color:#94A3B8>Umum</em>'}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
      </div>
      ${_csvParsedRows.length > 5 ? `<div style="font-size:11px;color:#64748B;margin-top:6px">
        ... dan ${_csvParsedRows.length - 5} baris lainnya</div>` : ''}
    </div>
    <div style="display:flex;gap:8px">
      <button class="btn btn--ghost" style="flex:1" onclick="document.getElementById('modal-import-csv').remove()">
        Batal
      </button>
      <button class="btn btn--primary" style="flex:2" id="btn-execute-import" onclick="executeImportCSV()">
        <div class="spinner-btn"></div>
        <span class="btn-text">✅ Import ${_csvParsedRows.length} Karyawan</span>
      </button>
    </div>`;
}
...
async function executeImportCSV() {
  ...
  if (_csvParsedRows.length > 500) {
    showToast('Maksimal 500 karyawan per import. Bagi jadi beberapa file.', 'error');
    return;
  }
  ...
  try {
    const result = await callAPI('bulkImportKaryawan', { rows: _csvParsedRows });
    _renderImportResult(result);
    // Refresh tabel karyawan
    if (typeof loadKaryawanTabel === 'function') loadKaryawanTabel();
  } catch(e) {
    showToast('Import gagal: ' + e.message, 'error', 8000);
    if (btn) { btn.disabled = false; btn.classList.remove('loading'); }
  }
}
...
function _renderImportResult(result) {
  ...
  const el = document.getElementById('csv-step-2');
  const resEl = document.getElementById('csv-result');
  el.style.display = 'block';

  const success = result.berhasil?.length || 0;
  const skipped = result.dilewati?.length || 0;
  const errors  = result.error?.length || 0;

  let html = `<div style="background:#ECFDF5;border:1px solid #10B981;border-radius:10px;
    padding:16px;margin-bottom:12px">
    <div style="font-size:20px;font-weight:700;color:#065F46;margin-bottom:8px">
      ✅ Import Selesai
    </div>
    <div style="font-size:13px;color:#065F46;line-height:1.7">
      • <strong>${success}</strong> karyawan berhasil ditambahkan<br>
      • <strong>${skipped}</strong> baris dilewati (duplikat)<br>
      • <strong>${errors}</strong> baris error
    </div>
    <div style="margin-top:10px;padding:8px;background:#fff;border-radius:6px;font-size:12px;color:#065F46">
      💡 <strong>Password default:</strong> <code>Absensi@123</code>
      — minta karyawan segera ganti password setelah login pertama
    </div>
  </div>`;

  if (success > 0) {
    html += `<details style="margin-bottom:8px">
      <summary style="cursor:pointer;padding:8px;background:#F0FDF4;border-radius:6px;font-size:13px;font-weight:600;color:#065F46">
        ✅ ${success} Berhasil ditambahkan (klik untuk lihat)
      </summary>
      <div style="max-height:200px;overflow-y:auto;padding:8px;background:#fff;border:1px solid #E2E8F0;border-radius:0 0 6px 6px">
        ${result.berhasil.map(b => `<div style="padding:4px 0;border-bottom:1px solid #F1F5F9;font-size:12px">
          <strong>${b.nama}</strong> — username: <code>${b.username}</code>, NIK: ${b.nik}
        </div>`).join('')}
      </div>
    </details>`;
  }
  if (skipped > 0) {
    html += `<details style="margin-bottom:8px">
      <summary style="cursor:pointer;padding:8px;background:#FEF3C7;border-radius:6px;font-size:13px;font-weight:600;color:#92400E">
        ⏭️ ${skipped} Dilewati (duplikat)
      </summary>
      <div style="max-height:200px;overflow-y:auto;padding:8px;background:#fff;border:1px solid #E2E8F0;border-radius:0 0 6px 6px">
        ${result.dilewati.map(b => `<div style="padding:4px 0;border-bottom:1px solid #F1F5F9;font-size:12px">
          Baris ${b.baris}: ${b.nama || '-'} — <em style="color:#92400E">${b.alasan}</em>
        </div>`).join('')}
      </div>
    </details>`;
  }
  if (errors > 0) {
    html += `<details style="margin-bottom:8px">
      <summary style="cursor:pointer;padding:8px;background:#FEE2E2;border-radius:6px;font-size:13px;font-weight:600;color:#991B1B">
        ❌ ${errors} Error
      </summary>
      <div style="max-height:200px;overflow-y:auto;padding:8px;background:#fff;border:1px solid #E2E8F0;border-radius:0 0 6px 6px">
        ${result.error.map(b => `<div style="padding:4px 0;border-bottom:1px solid #F1F5F9;font-size:12px">
          Baris ${b.baris}: ${b.nama || '-'} — <em style="color:#991B1B">${b.alasan}</em>
        </div>`).join('')}
      </div>
    </details>`;
  }

  html += `<button class="btn btn--primary btn--full" style="margin-top:12px"
    onclick="document.getElementById('modal-import-csv').remove()">Tutup</button>`;

  resEl.innerHTML = html;
  showToast(`✅ ${success} karyawan ditambahkan`, 'success', 5000);
}
