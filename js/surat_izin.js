...
// ── PROGRESS STEPPER — tampilkan progres TTD ─────────────────
function _progressBarSI(s) {
  const status = s.status_surat || '';
  const selesai = status === 'selesai';

  // Bangun daftar langkah sesuai pihak yang terlibat
  const steps = [];
  steps.push({ key: 'menunggu_karyawan', label: 'Karyawan', nama: s.nama_karyawan || '', done: !!s.ttd_karyawan });
  if (s.nama_atasan && String(s.nama_atasan).trim())
    steps.push({ key: 'menunggu_atasan',   label: 'Atasan',   nama: s.nama_atasan   || '', done: !!s.ttd_atasan });
  if (s.nama_pimpinan && String(s.nama_pimpinan).trim())
    steps.push({ key: 'menunggu_pimpinan', label: 'Pimpinan', nama: s.nama_pimpinan || '', done: !!s.ttd_pimpinan });
  steps.push({ key: 'menunggu_admin', label: 'Disetujui', nama: '', done: selesai });

  const stepWidth = Math.floor(100 / steps.length);

  const items = steps.map((step, i) => {
    const isDone    = step.done || selesai;
    const isActive  = !selesai && status === step.key;
    const isWaiting = !isDone && !isActive;

    const circleColor = isDone ? '#059669' : isActive ? '#1E5F3A' : '#CBD5E0';
    const textColor   = isDone ? '#059669' : isActive ? '#1E5F3A' : '#94A3B8';
    const icon        = isDone ? '✓' : isActive ? (i + 1) : (i + 1);
    const connector   = i < steps.length - 1
      ? `<div style="flex:1;height:2px;margin-top:-10px;background:${isDone ? '#059669' : '#E2E8F0'}"></div>`
      : '';

    return `
      <div style="display:flex;flex-direction:column;align-items:center;flex:1;min-width:0">
        <div style="width:24px;height:24px;border-radius:50%;
          background:${isDone ? '#059669' : isActive ? '#1E5F3A' : '#F1F5F9'};
          border:2px solid ${circleColor};
          display:flex;align-items:center;justify-content:center;
          font-size:11px;font-weight:700;color:${isDone || isActive ? '#fff' : '#94A3B8'};
          position:relative;z-index:1;flex-shrink:0">
          ${isDone ? '✓' : (i + 1)}
        </div>
        <div style="font-size:10px;font-weight:600;color:${textColor};
          margin-top:4px;text-align:center;line-height:1.3;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:60px">
          ${step.label}
        </div>
        ${isActive ? `<div style="font-size:9px;color:#1E5F3A;text-align:center;margin-top:1px">← sekarang</div>` : ''}
      </div>
      ${connector}`;
  });

  return `<div style="display:flex;align-items:flex-start;padding:10px 8px 6px;
    background:#F8FAFC;border-radius:8px;margin:10px 0;gap:0">
    ${items.join('')}
  </div>`;
}
...
function _cardSuratIzin(s, isAdmin = false) {
  ...
  const sudahSemua = s.status_surat === 'menunggu_admin' || s.status_surat === 'selesai';
  return `
  <div style="background:#fff;border-radius:12px;border:1px solid #E2E8F0;
    ...
    <div style="margin:6px 0 10px;padding:10px;background:#F8FAFC;border-radius:8px;
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap">
      ...
      ${isAdmin && sudahSemua && s.status_surat === 'menunggu_admin' ? `
    ...
    <div style="padding:20px;font-size:13px;color:#0F172A">
      ...
      <div style="display:flex;gap:8px;margin-top:16px">
        ${(()=>{
          ...
          const isAdm = sess && (sess.role==='admin'||sess.role==='superadmin');
          const bisaSetujui = isAdm && s.status_surat==='menunggu_admin';
          return bisaSetujui ? `
          <button onclick="document.getElementById('modal-surat-izin').remove();_setujuiSuratIzin('${idSurat}')"
            style="flex:2;padding:10px;background:#059669;color:#fff;border:none;
            border-radius:8px;font-size:13px;font-weight:700;cursor:pointer">
            ✅ Setujui Surat Izin
          </button>` : '';
        })()}
