  ...
  <div style="background:#fff;border-radius:16px;padding:0;
    ...
    <div style="padding:20px;font-size:13px;color:#0F172A">
      ...
      <div style="background:#F8FAFC;border-radius:8px;padding:10px;margin-bottom:14px">
        <div><span style="color:#64748B;width:100px;display:inline-block">Nama</span>:
          <strong>${s.nama_pimpinan && s.nama_pimpinan.trim() ? s.nama_pimpinan : s.nama_pembuat || '-'}</strong></div>
        <div><span style="color:#64748B;width:100px;display:inline-block">Jabatan</span>:
          ${s.nama_pimpinan && s.nama_pimpinan.trim() ? 'Project Manager' : (s.jabatan_pembuat || '-')}</div>
        <div><span style="color:#64748B;width:100px;display:inline-block">Instansi</span>: PT. HUTAKALO MINATANI PRIMA</div>
      </div>

      <div style="font-size:11px;color:#64748B;margin-bottom:4px">Memberikan izin kepada karyawan:</div>
      <div style="background:#F8FAFC;border-radius:8px;padding:10px;margin-bottom:14px">
...
async function _cetakSuratIzinPDF(idSurat) {
  ...
  try {
    ...
    const namaPemberiIzin = (s.nama_pimpinan && String(s.nama_pimpinan).trim())
      ? s.nama_pimpinan : (s.nama_pembuat || '-');
    const jabPemberiIzin  = (s.nama_pimpinan && String(s.nama_pimpinan).trim())
      ? 'Project Manager' : (s.jabatan_pembuat || '-');

    const rows1 = [
      ['Nama',    namaPemberiIzin],
      ['Jabatan', jabPemberiIzin],
    ...
    y += 2;
    doc.text('Dengan ini memberikan izin kepada karyawan:', mL, y); y += 7;
    ...
    const penutup = doc.splitTextToSize(
      ...
      'Karyawan yang bersangkutan diharapkan hadir kembali sesuai tanggal yang telah ditentukan.',
      W-mL-mR);
    doc.text(penutup, mL, y); y += penutup.length * 6 + 4;

    doc.setTextColor(100,100,100);
    doc.text('Diterbitkan di : ' + (inst.alamat_instansi ? inst.alamat_instansi.split(',')[0].trim() : 'Gorontalo Utara'), mL, y); y += 6;
